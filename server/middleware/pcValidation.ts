...
/**
 * Pure validation core, usable both by the Express middleware below and
 * by the CSV import pipeline (see utils/importTables.ts), so manually
 * entered and imported PC registrations are held to identical rules.
 * Throws an Error with a user-facing message on the first violation.
 */
export function validatePcRegistrationBody(body: Record<string, unknown>): void {
  const departmentId =
    typeof body.department_id === "string" ? body.department_id.trim() : "";
  if (!departmentId) {
    throw new Error(
      "Department is required for all PC registration entries.",
    );
  }
  ...
  if (host && !isValidHostname(host)) {
    throw new Error(
      "PC hostname must start with GBBIT01 and use letters, numbers, or hyphens only.",
    );
  }
  ...
  if (ip && !isValidIpv4(ip)) {
    throw new Error(
      "IP address must be a valid IPv4 address like 10.6.1.12.",
    );
  }
  ...
  const switchPort =
    body.switch_port_number === undefined ||
    body.switch_port_number === null ||
    body.switch_port_number === ""
      ? ""
  ...
  const switchName =
    typeof body.access_switch_name === "string"
      ? body.access_switch_name.trim()
      : "";
  if (switchName && !isValidAccessSwitchName(switchName)) {
    throw new Error(
      "Access switch name must be valid text without script content.",
    );
  }

  const patch =
    body.patch_level_number === undefined ||
    body.patch_level_number === null ||
    body.patch_level_number === ""
      ? ""
  ...
  const accessSwitchIp =
    typeof body.access_switch_ip === "string"
      ? body.access_switch_ip.trim()
      : "";
...
export function validatePcRegistration(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    validatePcRegistrationBody((req.body ?? {}) as Record<string, unknown>);
    next();
  } catch (error) {

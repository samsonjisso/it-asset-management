...
export function StdFieldsEditor({
  allKeys,
  meta,
  included,
  required,
  ...
  const remove = (key: string) => {
    onChange({
      included: included.filter((k) => k !== key),
  ...
  const saveRename = () => {
    ...
    if (!trimmed || trimmed === meta[renamingKey]?.label)
      delete nextLabels[renamingKey];
    ...
    <div className="space-y-2">
                ...
                {renamingKey === key ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      type="button"
                      onClick={saveRename}
                      className="w-5 h-5 flex items-center justify-center rounded-full hover:bg-brand-50 dark:hover:bg-brand-900/40 text-brand-600 shrink-0"
                      title="Save"
                    >
                      <Check size={12} />
                    </button>
                    <button
                      ...
                      className="w-5 h-5 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500 shrink-0"
                      title="Cancel"
                    >
                      <X size={12} />
                    </button>
      ...
      {included.length === 0 && (
        <p className="text-xs text-gray-400 dark:text-gray-500">
          No standard fields included — add some below.
        </p>
      )}

...
export const GET = withErrorHandling(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const row = await getRowById(serverTypesConfig, auth, id);
...
export const PATCH = withErrorHandling(async (req: NextRequest, { params }: { params: Promise<{ id: string }>  }) => {
  ...
  const { id } = await params;
  const row = await updateRow(serverTypesConfig, auth, id, body as Record<string, unknown>);
  return jsonOk(row, { headers: NO_STORE_HEADERS });
});
...
export const DELETE = withErrorHandling(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  const auth = await requireAuth(req);
  const { id } = await params;
  await deleteRow(serverTypesConfig, auth, id);

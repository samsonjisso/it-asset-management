...
export function AccessSwitchIpsPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from('access_switch_ips').select('*').order('label');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const conflict = ips.find((ip) => ip.id !== editing?.id && ip.label.trim() === normalized);
    if (conflict) {
      toast(`"${normalized}" is already registered as an access switch IP.`, 'error');
      return;
    }
    setSaving(true);
    const { error } = editing
      ? await supabase.from('access_switch_ips').update({ label: label.trim() }).eq('id', editing.id)
      : await supabase.from('access_switch_ips').insert({ label: label.trim() });
    setSaving(false);
    if (error) {
      toast(error.message, 'error');
    } else {
      toast(editing ? 'Access switch IP updated' : 'Access switch IP added', 'success');
      setModalOpen(false);
      loadData();
    }
  };
  ...
  const handleDelete = async (ip: AccessSwitchIp) => {
    ...
    const { error } = await supabase.from('access_switch_ips').delete().eq('id', ip.id);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-brand-600">Access Switch IP Management</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {ips.length} IP address{ips.length === 1 ? '' : 'es'} available on the PC Registration form
            </p>
          </div>
        </div>
        {canManage && (
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-10 text-center text-gray-500 dark:text-gray-400">
          No access switch IP addresses yet. Add one to make it available on the PC Registration form.
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {ips.map((ip) => (
            <div key={ip.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-4 gbb-card-hover">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800 dark:text-gray-100 font-mono">{ip.label}</p>
                  </div>
                </div>
                {canManage && (
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(ip)} className="p-1.5 text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/40 rounded-lg" title="Edit">
                      <Pencil size={16} />
                    </button>
                    <button onClick={() => handleDelete(ip)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Delete">
                      <Trash2 size={16} />
                    </button>
                  </div>
                )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Access Switch IP' : 'Add Access Switch IP'} size="sm">

...
export function validateReminderBody(body: Record<string, unknown>): void {
  if (
    typeof body.title === "string" &&
    body.title.trim() &&
    containsUnsafeScript(body.title)
  ) {
    throw new Error(
      "Reminder title contains invalid or unsafe script characters.",
    );
  }

  if (
    typeof body.reminder_type === "string" &&
    body.reminder_type.trim() &&
    containsUnsafeScript(body.reminder_type)
  ) {
    throw new Error(
      "Reminder type contains invalid or unsafe script characters.",
    );
  }

  const remindAt =
    typeof body.remind_at === "string" ? body.remind_at.trim() : "";
...
export function validateReminder(
  ...
  try {
    ...
    validateReminderBody((req.body ?? {}) as Record<string, unknown>);
    next();
  } catch (error) {

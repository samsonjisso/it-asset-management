...
export async function generateExcelReport(opts: ExcelReportOptions) {
  const { reportType, reportTitle, headers, rows, stats, periodLabel, generatedByName } = opts;
  ...
  const statPairs: [string, number][] = [
    ...
    ['Total Servers', stats.totalServers], ['Total Network Devices', stats.totalNetworkDevices],
  ...
  for (let i = 0; i < statPairs.length; i += 2) {
    ...
    row.getCell(1).value = statPairs[i][0];
    row.getCell(1).font = { color: { argb: 'FF5A6170' } };
    row.getCell(2).value = statPairs[i][1];
    row.getCell(2).font = { bold: true, color: { argb: NAVY_DARK } };
    if (statPairs[i + 1]) {
      row.getCell(3).value = statPairs[i + 1][0];
      row.getCell(3).font = { color: { argb: 'FF5A6170' } };
      row.getCell(4).value = statPairs[i + 1][1];
  ...
  const table = (title: string, items: { name: string; count: number }[]) => {
    ...
    if (items.length === 0) {
      ...
      summary.getCell(`A${r}`).font = { italic: true, color: { argb: 'FF9CA3AF' } };
      r += 1;
    } else {
      items.forEach((item, idx) => {
        ...
        if (idx % 2 === 0) row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: LIGHT } };
        r += 1;
      });
    }
    r += 1;
  ...
  (['active', 'expiringSoon', 'expired'] as const).forEach((k) => {
    const labels = { active: 'Active', expiringSoon: 'Expiring Soon (≤30 days)', expired: 'Expired' };
  ...
  dataSheet.columns = headers.map((h, i) => ({ header: h, key: `col${i}`, width: colWidths[i] }));
  ...
  rows.forEach((rowVals, i) => {
    const row = dataSheet.addRow(rowVals.map((v) => (v === null || v === undefined || v === '' ? '' : v)));
    row.height = 18;
    row.eachCell((cell) => {
  ...
  totalRow.getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: LIGHT_HEADER } };

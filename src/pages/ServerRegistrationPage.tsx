...
import { supabase, Server, ServerOwner, ServerType, ServerEnvironment, IPSubnet, OSRelease, HostLocation, Vendor, IPAddress, DirectoryUser } from '../lib/supabase';
...
import { Plus, Pencil, Trash2, Eye, Server as ServerIcon, Download, Upload } from 'lucide-react';
...
export function ServerRegistrationPage({ autoOpenCreate }: { autoOpenCreate?: number } = {}) {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const [serversRes, ownersRes, typesRes, envRes, subnetsRes, osReleasesRes, hostLocationsRes, vendorsRes, ipRes, employeesRes] = await Promise.all([
    ...
    if (hostLocationsRes.data) setHostLocations(hostLocationsRes.data as HostLocation[]);
  ...
  const matchedIp = useMemo(
    () => ipAddresses.find((ip) => ip.ip_address === form.ip_address.trim()) ?? null,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.os_release) {
      toast('OS Release is required. Add one under Customization > OS Releases.', 'error');
      return;
    }
    if (!form.host_location) {
      toast('Host Location is required. Add one under Customization > Host Locations.', 'error');
      return;
    }
    setSaving(true);
    const payload = {
  ...
  const exportCSV = () => {
    const headers = ['Asset ID', 'Server Type', 'Hostname', 'IP Address', 'Network Subnet', 'SSH Port', 'Environment', 'Owner', 'Vendor', 'RAM', 'CPU', 'Storage', 'OS Release', 'Host Location', 'Created At'];
    const rows = records.map((r) => [
      ...
      r.vendor ?? '',
      r.ram ?? '', r.cpu ?? '', r.storage ?? '',
      osReleases.find((rel) => rel.code === r.os_release)?.label ?? r.os_release ?? '',
      hostLocations.find((l) => l.code === r.host_location)?.label ?? r.host_location ?? '',
  ...
  const importColumns: ImportColumn[] = [
    { key: 'server_type', label: 'Server Type', required: true, example: serverTypes[0]?.label },
    { key: 'hostname', label: 'Hostname', required: true, example: 'SRV-HQ-001' },
    { key: 'ip_address', label: 'IP Address', required: true, example: '10.6.13.45' },
    { key: 'ssh_port', label: 'SSH Port', example: '22' },
    { key: 'environment', label: 'Environment', required: true, example: environments[0]?.label },
    { key: 'owner', label: 'Owner', required: true, example: serverOwners[0]?.label },
    { key: 'vendor', label: 'Vendor' },
    { key: 'ram', label: 'RAM', required: true, example: '32GB' },
    { key: 'cpu', label: 'CPU', required: true, example: 'Intel Xeon Silver 4210' },
    { key: 'storage', label: 'Storage', required: true, example: '2TB SSD' },
    { key: 'os_release', label: 'OS Release', required: true, example: osReleases[0]?.label },
    { key: 'host_location', label: 'Host Location', required: true, example: hostLocations[0]?.label },
  ...
  const validateImportRow = (raw: Record<string, string>) => {
    ...
    else if (!isValidIPv4(raw.ip_address)) errors.push('IP Address must be a valid IPv4 address');
    const sshPort = raw.ssh_port.trim() || '22';
    if (!isValidPort(sshPort)) errors.push('SSH Port must be a number between 1 and 65535');
    ...
    const detected = matchSubnet(raw.ip_address, subnets);
    const values = {
  ...
  const envColors: Record<string, string> = {
    ...
    standby: 'text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700',
  };
  const defaultEnvColor = 'text-brand-700 dark:text-brand-300 bg-brand-50 dark:bg-brand-900/40 border-brand-200';
  ...
  const serverSearchValue = (r: Server) => {
    ...
    return [
      r.asset_id, r.hostname, type?.label, r.server_type, r.server_type_other,
      env?.label, r.environment, owner?.label, r.server_owner, r.vendor,
      r.ip_address, r.network_subnet, r.ssh_port, r.ram, r.cpu, r.storage,
      os?.label, r.os_release, host?.label, r.host_location, r.notes,
    ].filter(Boolean).join(' ');
  };
  ...
  const columns: Column<Server>[] = [
    ...
    { key: 'vendor', label: 'Vendor', render: (r) => r.vendor ?? <span className="text-gray-300 dark:text-gray-600">-</span> },
    { key: 'network_subnet', label: 'Subnet', render: (r) => r.network_subnet ?? <span className="text-gray-300 dark:text-gray-600">-</span> },
  ...
  const viewSections: DetailSection[] = viewing ? [
    ...
    },
    {
      ...
      fields: [
        ...
        { label: 'OS Release', value: osReleases.find((r) => r.code === viewing.os_release)?.label ?? viewing.os_release },
        { label: 'Host Location', value: hostLocations.find((l) => l.code === viewing.host_location)?.label ?? viewing.host_location },
      ],
    },
    {
      title: 'Other',
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">{records.length} registered servers</p>
          </div>
        </div>
        ...
        <form noValidate onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Server Type" required hint={serverTypes.length === 0 ? "No server types configured yet — add one under Customization > Server Types." : undefined}>
              <SearchableSelect
                options={serverTypes.map((t) => ({ value: t.code, label: t.label }))}
                value={form.server_type}
                onChange={(val) => setForm({ ...form, server_type: val })}
                placeholder="Select server type"
                searchPlaceholder="Search server types…"
                emptyMessage={serverTypes.length === 0 ? 'No server types configured.' : 'No matching types.'}
                required
              />
            </Field>
              ...
              hint={
                form.ip_address
                  ? matchedIp
                    ...
                    : (detectedSubnet ? `Detected subnet: ${detectedSubnet.label}` : 'No matching subnet — add one under Customization > IP Subnets') + '. Not found in IP Management — this will be entered as a new address.'
            ...
            <Field label="SSH Port Number" required>
              <NumberInput value={form.ssh_port} onChange={(e) => setForm({ ...form, ssh_port: e.target.value })} placeholder="22" min={1} max={65535} required />
            </Field>
            <Field label="Server Environment" required hint={environments.length === 0 ? "No environments configured yet — add one under Customization > Server Environments." : undefined}>
              <SearchableSelect
                options={environments.map((env) => ({ value: env.code, label: env.label }))}
                value={form.environment}
                onChange={(val) => setForm({ ...form, environment: val })}
                placeholder="Select server environment"
                searchPlaceholder="Search environments…"
                emptyMessage={environments.length === 0 ? 'No environments configured.' : 'No matching environments.'}
                required
              />
            </Field>
            <Field label="Server Owner" required hint={serverOwners.length === 0 ? "No owners configured yet — an admin can add one under Server Owner Management." : undefined}>
              <SearchableSelect
                options={serverOwners.map((o) => ({ value: o.code, label: o.label }))}
                value={form.server_owner}
                onChange={(val) => setForm({ ...form, server_owner: val })}
                placeholder="Select server owner"
                searchPlaceholder="Search owners…"
                emptyMessage={serverOwners.length === 0 ? 'No server owners configured.' : 'No matching owners.'}
                required
              />
            </Field>
            <Field label="Vendor" hint={vendors.length === 0 ? "No vendors configured yet — add one under Customization > Vendors." : undefined}>
              <SearchableSelect
            ...
            <Field label="Resource RAM" required>
              <TextInput value={form.ram} onChange={(e) => setForm({ ...form, ram: e.target.value })} placeholder="e.g., 32GB" required />
            </Field>
            <Field label="Resource CPU" required>
              <TextInput value={form.cpu} onChange={(e) => setForm({ ...form, cpu: e.target.value })} placeholder="e.g., 8 cores" required />
            </Field>
            <Field label="Resource Storage" required>
              <TextInput value={form.storage} onChange={(e) => setForm({ ...form, storage: e.target.value })} placeholder="e.g., 1TB" required />
            </Field>
            <Field label="OS Release" required hint={osReleases.length === 0 ? "No OS releases configured yet — add one under Customization > OS Releases." : undefined}>
              <SearchableSelect
                options={osReleases.map((r) => ({ value: r.code, label: r.label }))}
                ...
                emptyMessage={osReleases.length === 0 ? 'No OS releases configured.' : 'No matching releases.'}
                required
              />
            </Field>
            <Field label="Host Location" required hint={hostLocations.length === 0 ? "No host locations configured yet — add one under Customization > Host Locations." : undefined}>
              <SearchableSelect
                options={hostLocations.map((l) => ({ value: l.code, label: l.label }))}
                ...
                emptyMessage={hostLocations.length === 0 ? 'No host locations configured.' : 'No matching locations.'}
                required
              />
            </Field>
          </div>
          <ImageInput value={form.image} onChange={(dataUrl) => setForm({ ...form, image: dataUrl })} label="Server Photo" hint="Optional — helps identify this server visually" variant="large" />
          <Field label="Notes">

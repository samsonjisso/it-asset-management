...
async function checkLabelDuplicate(
  ...
  if (rows.length > 0) {
    throw new ApiError(
      409,
      `${opts.itemLabel || "An entry"} "${value}" already exists.`,
    );
  }
}

function labelCodeHooks(
  table: string,
  fallbackCode: string,
...
export const departmentsConfig: CrudTableConfig = {
  ...
  beforeInsert: async (body, { conn }) => {
    if ("name" in body) {
      ...
      body.name = name;
      await checkLabelDuplicate(conn, "departments", "name", body, null, {
        itemLabel: "A department named",
      });
    }
  },
  beforeUpdate: async (body, { conn }, id) => {
    if ("name" in body) {
      ...
      if (!name) throw new ApiError(400, "Name is required");
      body.name = name;
      await checkLabelDuplicate(conn, "departments", "name", body, id, {
        itemLabel: "A department named",
      });
    }
  },
...
export const licenseTypesConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("license_types", "license_type", {
    itemLabel: "A license type named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM licenses WHERE license_type = ?",
      [row.code],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} license record(s) still use this type.`,
    );
  },
};
...
export const licenseSubtypesConfig: CrudTableConfig = {
  ...
  beforeInsert: async (body, { conn }) => {
    if (!body.label || !String(body.label).trim())
      throw new ApiError(400, "Label is required");
    if (!body.license_type_id)
      throw new ApiError(400, "License type is required");
    body.label = String(body.label).trim();
  ...
  beforeUpdate: async (body, { conn }, id) => {
    ...
    if ("label" in body || "license_type_id" in body) {
      const [rows] = await conn.query<any[]>(
        "SELECT label, license_type_id FROM license_subtypes WHERE id = ?",
        [id],
      );
      const current = rows[0];
      const label = "label" in body ? body.label : current?.label;
      const scopeVal =
        "license_type_id" in body
          ? body.license_type_id
          : current?.license_type_id;
      await checkLabelDuplicate(
        conn,
        "license_subtypes",
        "label",
        { label },
  ...
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM licenses WHERE license_subtype = ?",
      [row.label],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} license record(s) still use this subtype.`,
    );
  },
};
...
export const deviceTypesConfig: CrudTableConfig = {
  ...
  immutableCols: ["code"],
  ...labelCodeHooks("device_types", "device_type", {
    itemLabel: "A device type named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
...
export const deviceOwnersConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("device_owners", "device_owner", {
    itemLabel: "A device owner named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM devices WHERE device_owner = ?",
      [row.code],
    );
...
export const serverOwnersConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("server_owners", "server_owner", {
    itemLabel: "A server owner named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM servers WHERE server_owner = ?",
      [row.code],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} server record(s) still use this owner.`,
    );
  },
};
...
export const vendorsConfig: CrudTableConfig = {
  ...
  beforeDelete: async (row, { conn }) => {
    const a = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM licenses WHERE vendor = ?",
      [row.label],
    );
    const b = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM asset_models WHERE manufacturer = ?",
      [row.label],
    );
    const c3 = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM servers WHERE vendor = ?",
      [row.label],
    );
    const c = a + b + c3;
    return inUseError(
      c,
      `Cannot delete: ${c} record(s) still use this vendor.`,
    );
  },
};
...
export const serverTypesConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("server_types", "server_type", {
    itemLabel: "A server type named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM servers WHERE server_type = ?",
      [row.code],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} server record(s) still use this type.`,
    );
  },
};
...
export const serverEnvironmentsConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("server_environments", "environment", {
    itemLabel: "A server environment named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM servers WHERE environment = ?",
      [row.code],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} server record(s) still use this environment.`,
    );
  },
};
...
export const osReleasesConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("os_releases", "os_release", {
    itemLabel: "An OS release named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM servers WHERE os_release = ?",
      [row.code],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} server record(s) still use this OS release.`,
    );
  },
};
...
export const hostLocationsConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("host_locations", "host_location", {
    itemLabel: "A host location named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM servers WHERE host_location = ?",
      [row.code],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} server record(s) still use this host location.`,
    );
  },
};
...
export const floorsConfig: CrudTableConfig = {
  ...
  beforeInsert: async (body, { conn }) => {
    if (!body.label || !String(body.label).trim())
      throw new ApiError(400, "Label is required");
    body.label = String(body.label).trim();
    await checkLabelDuplicate(conn, "floors", "label", body, null, {
      itemLabel: "A floor named",
    });
    body.code =
      body.code && String(body.code).trim()
        ? String(body.code)
    ...
    if (
      ...
      const [rows] = await conn.query<any[]>(
        "SELECT MAX(position) as m FROM floors",
      );
  ...
  beforeUpdate: async (body, { conn }, id) => {
    if ("label" in body) {
      ...
      if (!label) throw new ApiError(400, "Label is required");
      body.label = label;
      await checkLabelDuplicate(conn, "floors", "label", body, id, {
        itemLabel: "A floor named",
      });
    }
  },
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM pc_registrations WHERE floor_number = ?",
      [row.label],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} PC record(s) still use this floor.`,
    );
  },
};
...
export const accessSwitchesConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("access_switches", "access_switch", {
    itemLabel: "An access switch named",
  }),
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM pc_registrations WHERE access_switch_name = ?",
      [row.label],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} PC record(s) still use this access switch.`,
    );
  },
};
...
async function validateAccessSwitchIp(
  body: Row,
  conn: PoolConnection,
  currentId: string | null,
): Promise<void> {
  ...
  if (!IPV4_RE.test(label))
    throw new ApiError(400, "Enter a valid IPv4 address (e.g. 10.6.1.103)");
  const [rows] = await conn.query<any[]>(
    "SELECT id FROM access_switch_ips WHERE TRIM(label) = TRIM(?) AND id != ?",
    [label, currentId || ""],
  );
  if (rows.length > 0)
    throw new ApiError(
      409,
      `IP Address ${label} is already registered as an access switch IP.`,
    );
  body.label = label;
...
export const accessSwitchIpsConfig: CrudTableConfig = {
  ...
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM pc_registrations WHERE access_switch_ip = ?",
      [row.label],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} PC record(s) still use this IP address.`,
    );
  },
};
...
export const patchLevelsConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  immutableCols: ["code"],
  ...labelCodeHooks("patch_levels", "patch_level", {
    itemLabel: "A patch/level value named",
  }),
  beforeDelete: async (row, { conn }) => {
    const usedByPc = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM pc_registrations WHERE patch_level_number = ?",
      [row.label],
    );
    const usedByIp = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM ip_addresses WHERE patch_panel_label = ?",
      [row.label],
    );
    ...
    return {
      status: 400,
      message: `Cannot delete: ${parts.join(" and ")} still use this value.`,
    };
  },
};
...
export const ipSubnetsConfig: CrudTableConfig = {
  ...
  deleteRoles: ["admin"],
  beforeInsert: async (body, { conn }) => {
    if (!body.prefix || !String(body.prefix).trim())
      throw new ApiError(400, "IP prefix is required");
    if (!body.label || !String(body.label).trim())
      throw new ApiError(400, "Label is required");
    body.prefix = String(body.prefix).trim();
    body.label = String(body.label).trim();
    await checkLabelDuplicate(conn, "ip_subnets", "prefix", body, null, {
      itemLabel: "A subnet with prefix",
    });
  },
  beforeUpdate: async (body, { conn }, id) => {
    if ("prefix" in body) {
      ...
      body.prefix = prefix;
      await checkLabelDuplicate(conn, "ip_subnets", "prefix", body, id, {
        itemLabel: "A subnet with prefix",
      });
    }
    if ("label" in body) {
...
export const assetModelsConfig: CrudTableConfig = {
  ...
  beforeInsert: async (body, { conn }) => {
    ...
    if (!["pc", "device"].includes(body.target))
      throw new ApiError(400, 'Target must be "pc" or "device"');
    body.name = String(body.name).trim();
    await checkLabelDuplicate(conn, "asset_models", "name", body, null, {
      itemLabel: "A model named",
      scopeCol: "target",
      scopeVal: body.target,
    });
  },
  beforeUpdate: async (body, { conn }, id) => {
    if ("name" in body) {
    ...
    if ("target" in body && !["pc", "device"].includes(body.target))
      throw new ApiError(400, 'Target must be "pc" or "device"');
    if ("name" in body || "target" in body) {
      const [rows] = await conn.query<any[]>(
        "SELECT name, target FROM asset_models WHERE id = ?",
        [id],
      );
      const current = rows[0];
      const name = "name" in body ? body.name : current?.name;
      const target = "target" in body ? body.target : current?.target;
      await checkLabelDuplicate(conn, "asset_models", "name", { name }, id, {
        itemLabel: "A model named",
        scopeCol: "target",
        scopeVal: target,
      });
  ...
  beforeDelete: async (row, { conn }) => {
    ...
    const c = await countUsage(
      conn,
      `SELECT COUNT(*) as c FROM ${table} WHERE model_id = ?`,
      [row.id],
    );
...
export const reminderTypesConfig: CrudTableConfig = {
  ...
  beforeInsert: async (body, { conn }) => {
    if (!body.label || !String(body.label).trim())
      throw new ApiError(400, "Label is required");
    body.label = String(body.label).trim();
    await checkLabelDuplicate(conn, "reminder_types", "label", body, null, {
      itemLabel: "A reminder type named",
    });
  },
  beforeUpdate: async (body, { conn }, id) => {
    if ("label" in body) {
      ...
      if (!label) throw new ApiError(400, "Label is required");
      body.label = label;
      await checkLabelDuplicate(conn, "reminder_types", "label", body, id, {
        itemLabel: "A reminder type named",
      });
    }
  },
  beforeDelete: async (row, { conn }) => {
    const c = await countUsage(
      conn,
      "SELECT COUNT(*) as c FROM reminders WHERE reminder_type = ?",
      [row.label],
    );
    return inUseError(
      c,
      `Cannot delete: ${c} reminder(s) still use this type.`,
    );
  },
};
...
async function validatePcLicense(
  body: Row,
  conn: PoolConnection,
  ...
  if (touchesLicenseId) {
    ...
    const [rows] = currentPcId
      ? await conn.query<any[]>(
          "SELECT license_id FROM pc_registrations WHERE id = ?",
          [currentPcId],
        )
      : [[]];
  ...
  const [licRows] = await conn.query<any[]>(
    "SELECT * FROM licenses WHERE id = ?",
    [licenseId],
  );
  ...
  if (!license)
    throw new ApiError(
      400,
      "Selected license was not found - it may have been deleted. Please choose another.",
    );
  ...
  if (conflict) {
    throw new ApiError(
      400,
      `This license is already assigned to PC "${conflict.hostname}". Unassign it there first, or choose a different license.`,
    );
  }
...
function duplicateAssetError(
  ...
  const idPart = existing.asset_id
    ? `Asset ID: ${existing.asset_id}`
  ...
  return new ApiError(
    409,
    `This ${itemLabel} is already registered in the system (matched by ${label} - ${idPart}${namePart}).`,
  );
}
...
const LICENSE_DUPLICATE_FIELDS: [string, string][] = [
  ["license_key", "License Key / Product Key"],
];

async function checkPcDuplicate(
  body: Row,
  conn: PoolConnection,
  currentId: string | null,
) {
  const match = await findDuplicateAsset(
    conn,
    "pc_registrations",
    PC_DUPLICATE_FIELDS,
    body,
...
async function checkDeviceDuplicate(
  body: Row,
  conn: PoolConnection,
  currentId: string | null,
) {
...
async function checkServerDuplicate(
  body: Row,
  conn: PoolConnection,
  currentId: string | null,
) {
...
async function checkAndLinkIp(
  ...
  if (!IPV4_RE.test(raw))
    throw new ApiError(
      400,
      "IP Address must be a valid IPv4 address (e.g., 10.6.13.45)",
    );

  const [ipRows] = await conn.query<any[]>(
...
async function attachIpRecord(row: Row, conn: Queryable): Promise<Row> {
  ...
  if (row.ip_id) {
    const [rows] = await (conn as any).query(
      "SELECT * FROM ip_addresses WHERE id = ?",
      [row.ip_id],
    );
...
function validateLicense(body: Row, isInsert: boolean): void {
  ...
  if (isInsert || "number_of_licenses" in body) {
    ...
    if (!value) {
      if (!isWindowsLicense)
        throw new ApiError(400, "Number of Licenses is required");
  ...
  if (isInsert || "attachment" in body) {
    ...
    if (!attachment) {
      ...
      if (!body.attachment_name || !String(body.attachment_name).trim())
        throw new ApiError(400, "Attachment filename is missing");
...
export const licensesConfig: CrudTableConfig = {
  ...
  beforeDelete: async (row, { conn }) => {
    const [rows] = await conn.query<any[]>(
      "SELECT hostname FROM pc_registrations WHERE license_id = ?",
      [row.id],
    );
    if (rows[0])
      return {
        status: 400,
        message: `Cannot delete: this license is assigned to PC "${rows[0].hostname}". Unassign it there first.`,
      };
    return null;
  },
  notify: {
    type: "License",
    label: (row) =>
      row.license_name || row.license_key || row.asset_id || row.id,
  },
};
...
export const pcRegistrationsConfig: CrudTableConfig = {
  ...
  decorate: async (row, conn) =>
    attachIpRecord(await attachLicenseToPc(row, conn), conn),
  notify: {
    type: "PC",
...
export const devicesConfig: CrudTableConfig = {
  ...
  notify: {
    type: "Device",
    label: (row) => row.hostname || row.device_model || row.asset_id || row.id,
  },
};
...
async function validateServer(
  ...
  if ("host_location" in body) {
    const [rows] = await conn.query<any[]>(
      "SELECT 1 FROM host_locations WHERE code = ?",
      [body.host_location],
    );
    if (!rows[0])
      throw new ApiError(400, "Selected Host Location was not found");
  }
}
...
const IP_HOSTNAME_RE =
  /^(?!-)[A-Za-z0-9-]{1,63}(?<!-)(\.(?!-)[A-Za-z0-9-]{1,63}(?<!-))*$/;
const IP_STATUS_VALUES = [
  "assigned",
  "reserved",
  "available",
  "decommissioned",
...
function findMatchingSubnet(ip: string, subnets: Row[]): Row | null {
  ...
  for (const s of subnets) {
    ...
    if (
      ...
      (!best || prefix.length > String(best.prefix).trim().length)
    )
      best = s;
  }
  return best;
...
async function validateIpAddress(
  ...
  const [configRows] = await conn.query<any[]>(
    "SELECT required_base_fields, fields FROM ip_form_fields ORDER BY created_at LIMIT 1",
  );
  ...
  if (config?.required_base_fields) {
    try {
      const parsed =
        typeof config.required_base_fields === "string"
  ...
  if ("ip_address" in body && !IPV4_RE.test(body.ip_address))
    throw new ApiError(
      400,
      "IP Address must be a valid IPv4 address (e.g., 10.6.1.50)",
    );
  if ("ip_address" in body) {
    const [rows] = await conn.query<any[]>(
      "SELECT id, hostname FROM ip_addresses WHERE ip_address = ? AND id != ?",
      [body.ip_address, currentId || ""],
    );
    if (rows[0]) {
      throw new ApiError(
        409,
        `IP Address ${body.ip_address} is already registered${rows[0].hostname ? ` to "${rows[0].hostname}"` : ""}. Choose a different address.`,
      );
    }
  }
  if ("mac_address" in body && !IP_MAC_RE.test(body.mac_address))
    throw new ApiError(400, "MAC Address must look like 00:1A:2B:3C:4D:5E");
  if ("hostname" in body && !IP_HOSTNAME_RE.test(body.hostname)) {
    throw new ApiError(
      400,
      "Hostname may only contain letters, numbers, hyphens and dots (e.g., PC-HQ-001)",
    );
  }
  if ("status" in body) {
    if (body.status === "unassigned")
      throw new ApiError(
        400,
        "Select a status (other than Unassigned) to complete registration",
      );
    if (!IP_STATUS_VALUES.includes(body.status))
      throw new ApiError(400, "Select a valid status");
  }
  if ("department_id" in body) {
    ...
    if (!rows[0])
      throw new ApiError(400, "Selected department/branch was not found");
  }
  if ("patch_panel_label" in body) {
    const [rows] = await conn.query<any[]>(
      "SELECT 1 FROM patch_levels WHERE label = ?",
      [body.patch_panel_label],
    );
    if (!rows[0])
      throw new ApiError(
        400,
        "Selected Patch Panel Label / Number was not found. Configure one under Customization > Patch / Level Numbers.",
      );
  }
  if ("subnet_id" in body) {
    const [subnetRows] = await conn.query<any[]>(
      "SELECT * FROM ip_subnets WHERE id = ?",
      [body.subnet_id],
    );
    ...
    if (!subnet)
      throw new ApiError(
        400,
        "Selected IP Subnet was not found. Configure one under Customization > IP Subnets.",
      );
    if ("ip_address" in body) {
      const prefix = (subnet.prefix || "").trim();
      if (!prefix || !body.ip_address.startsWith(prefix))
        throw new ApiError(
          400,
          `IP Address does not belong to the selected subnet (${subnet.prefix}).`,
        );
      const [allSubnets] = await conn.query<any[]>("SELECT * FROM ip_subnets");
      const best = findMatchingSubnet(body.ip_address, allSubnets);
      if (best && best.id !== subnet.id)
        throw new ApiError(
          400,
          `IP Address matches a different configured subnet (${best.prefix}). Select that subnet instead.`,
        );
    }
  }
  if ("ip_address" in body) {
    ...
    if (existingRows[0] && existingRows[0].id !== currentId) {
      throw new ApiError(
        400,
        "This IP Address is already registered/assigned. Choose an available address from the selected subnet.",
      );
    }
  }
}
...
export const ipAddressesConfig: CrudTableConfig = {
  ...
  beforeUpdate: (body, { conn }, id) =>
    validateIpAddress(body, conn, false, id),

...
const ACTION_STYLE: Record<AdminNotification['action'], { label: string; color: string; icon: JSX.Element }> = {
  update: { label: 'Updated', color: 'text-brand-700 dark:text-brand-300 bg-brand-50 dark:bg-brand-900/40 border border-brand-200', icon: <Pencil size={12} /> },
  delete: { label: 'Deleted', color: 'text-red-700 bg-red-50 border border-red-200', icon: <Trash2 size={12} /> },
};
...
export function NotificationsPage() {
  ...
  const markRead = async (rec: AdminNotification) => {
    const { error } = await supabase.from('notifications').update({ is_read: true }).eq('id', rec.id);
    if (error) toast(error.message, 'error');
    else setRecords((prev) => prev.map((r) => (r.id === rec.id ? { ...r, is_read: true } : r)));
  };
  ...
  const handleDelete = async (rec: AdminNotification) => {
    ...
    const { error } = await supabase.from('notifications').delete().eq('id', rec.id);
    if (error) toast(error.message, 'error');
    else {
  ...
  const columns: Column<AdminNotification>[] = [
    {
      ...
      render: (r) => {
        ...
        return (
          <span className={`text-xs px-2 py-1 rounded-full font-medium inline-flex items-center gap-1 ${style.color}`}>
            {style.icon}
            {style.label}
          </span>
        );
    ...
    },
    {
      ...
      render: (r) => (
        <div>
          <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{r.record_label || '—'}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">{r.record_type}</p>
        </div>
      ),
    },
    {
      key: 'summary',
      label: 'What Changed',
      render: (r) => <span className="text-sm text-gray-600 dark:text-gray-300 line-clamp-2">{r.summary}</span>,
    },
    {
      key: 'actor_name',
      label: 'Performed By',
      ...
      render: (r) => <span className="text-sm text-gray-700 dark:text-gray-300">{r.actor_name}</span>,
    },
    {
      key: 'created_at',
      label: 'Date & Time',
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {records.length} notification{records.length === 1 ? '' : 's'}, {unreadCount} unread
            </p>
          </div>
        </div>
        {unreadCount > 0 && (
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 border border-brand-600 rounded-xl px-4 py-3 flex items-start gap-2">
        ...
        Every time an important record — an asset, IP address, device, server, license, PC registration, or user
        account — is updated or deleted, it shows up here with who made the change, what changed, which record was
        affected, and when.
      </p>

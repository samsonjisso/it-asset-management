...
import { supabase, License, LicenseType, LicenseSubtype, Vendor, Reminder, DirectoryUser } from '../lib/supabase';
...
import { Plus, Pencil, Trash2, Eye, KeyRound, Download, Upload, AlertTriangle, CheckCircle, FileText, Paperclip, BellRing } from 'lucide-react';
...
export function LicenseRegistrationPage({ autoOpenCreate }: { autoOpenCreate?: number } = {}) {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const [licRes, typesRes, subtypesRes, vendorsRes, employeesRes] = await Promise.all([
  ...
  const openEdit = async (rec: License) => {
    ...
    const { data } = await supabase.from('reminders').select('*').eq('license_id', rec.id);
    const existing = ((data as Reminder[] | null) ?? [])[0] ?? null;
    if (existing) {
      const days = rec.expiry_date
        ? Math.round((new Date(`${rec.expiry_date}T00:00:00`).getTime() - new Date(existing.remind_at).getTime()) / (1000 * 60 * 60 * 24))
  ...
  const isWindowsLicense = form.license_subtype.trim().toLowerCase() === 'windows';
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    } else if (form.number_of_licenses.trim() && !/^\d+$/.test(form.number_of_licenses.trim())) {
      toast('Number of Licenses must be a whole number', 'error');
      return;
    }
    if (!isWindowsLicense && !form.never_expires) {
      if (!form.expiry_date) {
        toast('Expiry Date is required unless Never Expire is selected', 'error');
        return;
      }
    ...
    const wantsAutoReminder = !isWindowsLicense && !form.never_expires && form.auto_reminder;
    if (wantsAutoReminder) {
      const daysNum = parseInt(form.auto_reminder_days, 10);
      if (!form.auto_reminder_days.trim() || !Number.isFinite(daysNum) || daysNum < 1) {
        toast('Days Before Expiry must be a whole number of 1 or more', 'error');
        return;
      }
    }
    setSaving(true);
    const payload = {
      ...
      expiry_date: isWindowsLicense || form.never_expires ? null : form.expiry_date,
  ...
  // secondary toast.
  const syncAutoReminder = async (licenseId: string, expiryDate: string | null, wantsReminder: boolean) => {
    try {
      if (!wantsReminder || !expiryDate) {
        if (linkedReminderId) await supabase.from('reminders').delete().eq('id', linkedReminderId);
        return;
      }
      ...
      if (linkedReminderId) {
        ...
        await supabase.from('reminders').update(reminderPayload).eq('id', linkedReminderId);
      } else {
        await supabase.from('reminders').insert(reminderPayload);
      }
    } catch {
      toast('License saved, but the auto-reminder could not be synced', 'error');
    }
  };

  const handleDelete = async (rec: License) => {
    if (!confirm(`Delete license "${rec.license_name || rec.license_subtype || rec.license_type}"?`)) return;
  ...
  const getExpiryStatus = (rec: License) => {
    ...
    if (days <= 60) return { label: `Expires in ${days}d`, color: 'text-brand-600 bg-brand-50 dark:bg-brand-900/40', days };
    return { label: 'Active', color: 'text-green-600 bg-green-50', days };
  };

  const exportCSV = () => {
    const headers = ['Asset ID', 'License Name', 'License Type', 'Subtype', 'Vendor', 'License Key', 'Number of Licenses', 'Effective Date', 'Expiry Date', 'Status', 'Created At'];
    const rows = records.map((r) => {
      const status = getExpiryStatus(r);
      return [
        r.asset_id ?? '', r.license_name ?? '', r.license_type, r.license_subtype ?? '', r.vendor ?? '', r.license_key ?? 'N/A',
  ...
  const parseDateCell = (raw: string): { value: string | null; error?: string } => {
    ...
    if (isNaN(d.getTime())) return { value: null, error: `"${raw}" isn't a recognizable date` };
    return { value: d.toISOString().slice(0, 10) };
  };

  const importColumns: ImportColumn[] = [
    { key: 'license_name', label: 'License Name', required: true, example: 'Adobe Acrobat Pro' },
    { key: 'license_type', label: 'License Type', required: true, example: licenseTypeOptions[0]?.label },
    { key: 'license_subtype', label: 'Subtype', required: true, example: 'Standard' },
  ...
  const validateImportRow = (raw: Record<string, string>) => {
    ...
    if (!raw.license_type) errors.push('License Type is required');
      ...
      const t = findByLabel(licenseTypeOptions, (x) => x.label, raw.license_type);
      if (!t) errors.push(`License Type "${raw.license_type}" not found`);
      else { typeCode = t.code; typeId = t.id; }
    }
    let subtypeLabel: string | null = null;
    let isWindows = false;
    if (!raw.license_subtype) errors.push('Subtype is required');
    else if (typeId) {
      const sub = licenseSubtypes.find((s) => s.license_type_id === typeId && s.label.trim().toLowerCase() === raw.license_subtype.trim().toLowerCase());
      if (!sub) errors.push(`Subtype "${raw.license_subtype}" isn't configured for License Type "${raw.license_type}"`);
      else { subtypeLabel = sub.label; isWindows = sub.label.trim().toLowerCase() === 'windows'; }
    }
    ...
    if (numRaw && !/^\d+$/.test(numRaw)) errors.push('Number of Licenses must be a whole number');
    else if (!isWindows && !numRaw) errors.push('Number of Licenses is required');

    const neverExpiresRaw = raw.never_expires.trim().toLowerCase();
    const neverExpires = isWindows || ['yes', 'y', 'true', '1'].includes(neverExpiresRaw);

    const effective = parseDateCell(raw.effective_date);
    if (effective.error) errors.push(`Effective Date: ${effective.error}`);
    const expiry = parseDateCell(raw.expiry_date);
    if (expiry.error) errors.push(`Expiry Date: ${expiry.error}`);
    if (!isWindows && !neverExpires) {
      if (!expiry.value) errors.push('Expiry Date is required unless Never Expire is set to Yes');
      else if (effective.value && expiry.value < effective.value) errors.push('Expiry Date cannot be before the Effective Date');
    }
  ...
  const licenseSearchValue = (r: License) => {
    const typeLabel = licenseTypeOptions.find((t) => t.code === r.license_type)?.label;
    return [
      r.asset_id, typeLabel, r.license_type, r.license_subtype, r.vendor,
      r.license_key, r.number_of_licenses, r.assigned_pc?.hostname, r.notes, r.license_name,
    ].filter(Boolean).join(' ');
  };
  ...
  const columns: Column<License>[] = [
    ...
    { key: 'license_name', label: 'Name', sortable: true, sortValue: (r) => r.license_name ?? '', render: (r) => (
      <div className="flex flex-col">
        <span className="font-medium text-gray-900 dark:text-gray-100">{r.license_name || licenseTypeOptions.find((t) => t.code === r.license_type)?.label || r.license_type}</span>
        <span className="text-xs text-gray-400 dark:text-gray-500">{licenseTypeOptions.find((t) => t.code === r.license_type)?.label ?? r.license_type}{r.license_subtype ? ` — ${r.license_subtype}` : ''}</span>
      </div>
    )},
    { key: 'vendor', label: 'Vendor', render: (r) => r.vendor ?? '-' },
    {
      key: 'assigned_pc',
      label: 'Assigned',
      render: (r) => r.assigned_pc
        ? <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300">{r.assigned_pc.hostname}</span>
    ...
    },
    {
      ...
      render: (r) => {
        const status = getExpiryStatus(r);
        if (!r.expiry_date) return <span className="text-gray-400 dark:text-gray-500">No expiry</span>;
        return (
  ...
  const viewSections: DetailSection[] = viewing ? [
    {
      ...
      fields: [
        ...
        { label: 'Assigned PC', value: viewing.assigned_pc ? `${viewing.assigned_pc.hostname}${viewing.assigned_pc.asset_id ? ` (${viewing.assigned_pc.asset_id})` : ''}` : 'Not assigned' },
      ],
    },
    {
      title: 'Validity',
      ...
      fields: [
        { label: 'Notes', value: viewing.notes, full: true },
        {
          ...
          value: viewing.attachment ? (
            ...
            >
              {viewing.attachment.startsWith('data:image/') ? <FileText size={14} /> : <Paperclip size={14} />}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">{records.length} registered licenses</p>
          </div>
        </div>
      ...
      <DetailsModal
        ...
        title={viewing ? (viewing.license_name || licenseTypeOptions.find((t) => t.code === viewing.license_type)?.label || viewing.license_type) : ''}
        ...
        <form noValidate onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="License Type" required>
              <SearchableSelect
                options={licenseTypeOptions.map((t) => ({ value: t.code, label: t.label }))}
                value={form.license_type}
                onChange={(val) => setForm({ ...form, license_type: val, license_subtype: '' })}
                placeholder="Select license type"
                searchPlaceholder="Search license types…"
                emptyMessage={licenseTypeOptions.length === 0 ? 'No license types configured.' : 'No matching types.'}
                required
              />
            </Field>
            <Field label="License Subtype" required hint={currentSubtypes.length === 0 ? 'No subtypes configured for this license type — ask an admin to add one' : undefined}>
              <SearchableSelect
                options={currentSubtypes.map((s) => ({ value: s.label, label: s.label }))}
                value={form.license_subtype}
                onChange={(val) => setForm({ ...form, license_subtype: val })}
                placeholder="Select subtype"
                searchPlaceholder="Search subtypes…"
                emptyMessage={currentSubtypes.length === 0 ? 'No subtypes configured.' : 'No matching subtypes.'}
                required
              />
            </Field>
            <Field label="Vendor (Company)" hint={vendors.length === 0 ? 'No vendors configured yet — add one under Customization > Vendors.' : undefined}>
              <SearchableSelect
            ...
            <Field label="Number of Licenses" required={!isWindowsLicense} hint={isWindowsLicense ? 'Not required for a Windows license' : 'Numbers only'}>
              <NumberInput min={0} value={form.number_of_licenses} onChange={(e) => setForm({ ...form, number_of_licenses: e.target.value })} placeholder="Total purchased licenses" required={!isWindowsLicense} />
            </Field>
            {!isWindowsLicense && (
            ...
            {!isWindowsLicense && (
              <Field label="Expiry Date" required={!form.never_expires} hint={form.never_expires ? 'This license does not expire' : undefined}>
            ...
            <div className="rounded-xl border border-brand-600 bg-gray-50 dark:bg-gray-900 p-4 space-y-3">
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 select-none">
                <input
                  ...
                  onChange={(e) => setForm({ ...form, auto_reminder: e.target.checked })}
                  className="rounded border-gray-300 dark:border-gray-600 text-brand-600 focus:ring-brand-500"
                />
              ...
              {form.auto_reminder && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pl-6">
                  <Field label="Days Before Expiry" required hint="A reminder will appear in Reminders & Notifications on this date">
                    <NumberInput
                      min={1}
                      value={form.auto_reminder_days}
                      onChange={(e) => setForm({ ...form, auto_reminder_days: e.target.value })}
                      placeholder="30"
                      required
                    />
                  </Field>
                  <Field label="Alert Email" hint="Optional — sends an email alert too, not just the in-app reminder">
                    <TextInput
                      type="email"
                      value={form.auto_reminder_email}
                      onChange={(e) => setForm({ ...form, auto_reminder_email: e.target.value })}
          ...
          <FileAttachmentInput
            ...
            onChange={(dataUrl, fileName) => setForm({ ...form, attachment: dataUrl, attachment_name: fileName })}

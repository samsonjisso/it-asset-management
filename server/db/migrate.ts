...
async function main() {
  ...
  for (const column of pcJsonColumns) {
    ...
    await connection.query(
      `ALTER TABLE pc_form_fields ADD COLUMN \`${column}\` JSON NULL`,
    );
    await connection.query(
      `UPDATE pc_form_fields SET \`${column}\` = ? WHERE \`${column}\` IS NULL`,
      ["[]"],
    );
    await connection.query(
      `ALTER TABLE pc_form_fields MODIFY COLUMN \`${column}\` JSON NOT NULL`,
    );
  }
  ...
  const existingIpConfigColumns = new Set(
    ipConfigColumns.map((row) => row.COLUMN_NAME),
  );
  for (const column of pcJsonColumns) {
    if (existingIpConfigColumns.has(column)) continue;
    await connection.query(
      `ALTER TABLE ip_form_fields ADD COLUMN \`${column}\` JSON NULL`,
    );
    await connection.query(
      `UPDATE ip_form_fields SET \`${column}\` = ? WHERE \`${column}\` IS NULL`,
      ["[]"],
    );
    await connection.query(
      `ALTER TABLE ip_form_fields MODIFY COLUMN \`${column}\` JSON NOT NULL`,
    );
  }
  ...
  if (
    !new Set(ipAddressColumns.map((row) => row.COLUMN_NAME)).has("extra_data")
  ) {
    await connection.query(

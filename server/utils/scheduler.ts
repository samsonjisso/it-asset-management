...
async function checkDueReminders() {
  try {
      ...
      for (const reminder of due) {
        try {
          ...
          const sent = await sendMail({
            to: reminder.alert_email as string,
            subject,
            text,
            html,
          });
          ...
          if (sent) {
            console.log(
              `[reminders] Sent alert email for "${reminder.title}" to ${reminder.alert_email}`,
            );
          }
        } catch (err) {
          console.error(
    ...
    console.error(
      "[reminders] Error checking due reminders:",
      (err as Error).message,
    );
  }
}
...
export function startReminderScheduler() {
  ...
  console.log(
    `[reminders] Email scheduler started (checking every ${Math.round(intervalMs / 1000)}s)`,
  );
}

...
export function VendorsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const { error } = editing
      ? await supabase.from('vendors').update({ label: label.trim() }).eq('id', editing.id)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-brand-600">Vendor Management</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {vendors.length} vendor{vendors.length === 1 ? '' : 's'} available on the License, Asset Model and Server Registration forms
            </p>
          </div>
        </div>
        {canManage && (
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {vendors.map((v) => (
            <div key={v.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-4 gbb-card-hover">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800 dark:text-gray-100">{v.label}</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500 font-mono">{v.code}</p>
                  </div>
                </div>
                {canManage && (
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(v)} className="p-1.5 text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/40 rounded-lg" title="Rename">
                      <Pencil size={16} />
                    </button>
                    <button onClick={() => handleDelete(v)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Delete">
                      <Trash2 size={16} />
                    </button>
                  </div>
                )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Rename Vendor' : 'Add Vendor'} size="sm">
        <form noValidate onSubmit={handleSave} className="space-y-4">
          <Field label="Name" required hint="e.g., Microsoft, Dell, Cisco, VMware">
            <TextInput value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g., Microsoft" required autoFocus />
          </Field>

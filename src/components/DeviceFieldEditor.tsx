...
import { FIELD_TYPE_OPTIONS, fieldTypeLabel, fieldTypeNeedsOptions } from '../lib/deviceTypeFields';
...
const emptyDraft: DraftField = { label: '', type: 'text', required: false, placeholder: '', optionsText: '' };

function slugifyKey(label: string, existing: string[]): string {
  let key = label.toLowerCase().trim().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
  if (!key) key = 'field';
...
// easy to scan.
export function DeviceFieldEditor({ fields, onChange }: DeviceFieldEditorProps) {
  ...
  const save = () => {
    ...
    const options = needsOptions ? optionsFromText(draft.optionsText) : undefined;
    ...
    if (mode === 'edit' && editingKey) {
      onChange(
        fields.map((f) =>
          f.key === editingKey
            ? { ...f, label, type: draft.type, required: draft.required, placeholder: draft.placeholder.trim() || undefined, options }
            : f
        )
      );
    } else {
      const key = slugifyKey(label, fields.map((f) => f.key));
      onChange([...fields, { key, label, type: draft.type, required: draft.required, placeholder: draft.placeholder.trim() || undefined, options }]);
    }
    cancel();
  };
    ...
    <div className="space-y-2">
        ...
        <div className="border border-brand-600 rounded-xl divide-y divide-gray-100 overflow-hidden">
          {fields.map((f, i) =>
            editingKey === f.key && mode === 'edit' ? (
              ...
              <div key={f.key} className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-900">
                <div className="flex flex-col -my-1 shrink-0">
                  ...
                  <button type="button" onClick={() => move(i, 1)} disabled={i === fields.length - 1} className="text-gray-300 dark:text-gray-600 hover:text-brand-600 disabled:opacity-30 disabled:hover:text-gray-300 dark:disabled:hover:text-gray-600" title="Move down">
                    <ChevronDown size={13} />
                  </button>
                </div>
                ...
                <button type="button" onClick={() => startEdit(f)} className="p-1.5 text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/40 rounded-lg shrink-0" title="Edit field">
                  <Pencil size={14} />
                </button>
                <button type="button" onClick={() => remove(f.key)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg shrink-0" title="Delete field">
                  <Trash2 size={14} />
                </button>
              </div>
            )
      ...
      {mode === 'add' ? (
        <div className="border border-brand-200 rounded-xl overflow-hidden">
          <FieldDraftForm draft={draft} setDraft={setDraft} needsOptions={needsOptions} onSave={save} onCancel={cancel} />
        </div>
      ) : (
      ...
      {fields.length === 0 && mode === 'idle' && (
        <p className="text-xs text-gray-400 dark:text-gray-500">No custom fields yet — add exactly what this device type needs.</p>
      )}
    </div>
  );
}
    ...
    <div className="p-3 bg-brand-50/40 space-y-2.5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <TextInput
          ...
          autoFocus
        />
        <SelectInput value={draft.type} onChange={(e) => setDraft({ ...draft, type: e.target.value as DeviceTypeField['type'] })}>
          {Object.entries(
            FIELD_TYPE_OPTIONS.reduce<Record<string, typeof FIELD_TYPE_OPTIONS>>((acc, o) => {
              (acc[o.group] ??= []).push(o);
              return acc;
            }, {})
          ).map(([group, opts]) => (
      ...
      {needsOptions && (
        <div>
          ...
          />
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">One choice per line — at least one is required.</p>
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onCancel} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500" title="Cancel">
            <X size={15} />
          </button>
          <button
            type="button"
            onClick={onSave}
            disabled={!draft.label.trim() || (needsOptions && optionsFromText(draft.optionsText).length === 0)}

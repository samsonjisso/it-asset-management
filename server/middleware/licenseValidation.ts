...
export function validateLicenseBody(body: Record<string, unknown>): void {
  ...
  const licenseType =
    typeof body.license_type === "string" ? body.license_type.trim() : "";
  ...
  if (
    numberOfLicenses !== undefined &&
    numberOfLicenses !== null &&
    numberOfLicenses !== "" &&
    !/^[0-9]+$/.test(String(numberOfLicenses))
  ) {
  ...
  if (
    ...
    !Date.parse(body.effective_date)
  ) {
...
export function validateLicense(
  ...
  try {
    ...
    validateLicenseBody((req.body ?? {}) as Record<string, unknown>);
    next();
  } catch (error) {

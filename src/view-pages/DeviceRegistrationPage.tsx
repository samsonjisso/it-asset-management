...
export function DeviceRegistrationPage({
  autoOpenCreate,
  onNavigate,
}: { autoOpenCreate?: number; onNavigate?: (page: string) => void } = {}) {
  ...
  const loadData = useCallback(async () => {
    ...
    if (modelsRes.data)
      setDeviceModels(
        (modelsRes.data as AssetModel[]).filter((m) => m.target === "device"),
      );
  ...
  const openAdd = () => {
    ...
    setForm({
      ...
      device_owner: deviceOwners[0]?.code ?? "",
    });
    setSkipIP(false);
    setModalOpen(true);
  };
  ...
  const employeeLabel = useCallback(
    (id: string) => employees.find((e) => e.id === id)?.full_name ?? id,
    [employees],
  );
  ...
  const modelsForType = useMemo(
    () =>
      deviceModels.filter(
        (m) => !m.device_type || m.device_type === form.device_type,
      ),
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    for (const key of [
      ...requiredCoreFields.filter((k) => coreFields.includes(k)),
      ...requiredBaseFields,
    ]) {
    ...
    const payload = {
      ...form,
      device_owner: coreFields.includes("device_owner")
        ? form.device_owner || null
        : null,
      department_id: coreFields.includes("department_id")
        ? form.department_id || null
        : null,
      device_model: coreFields.includes("device_model")
        ? form.device_model || null
        : null,
      hostname: coreFields.includes("hostname") ? form.hostname || null : null,
      ip_address:
        !skipIP && baseFields.includes("ip_address")
          ? form.ip_address || null
          : null,
      serial_number: baseFields.includes("serial_number")
        ? form.serial_number || null
        : null,
      mac_address: baseFields.includes("mac_address")
        ? form.mac_address || null
        : null,
      location: baseFields.includes("location") ? form.location || null : null,
      rack_number: baseFields.includes("rack_number")
        ? form.rack_number || null
        : null,
  ...
  const exportCSV = () => {
    ...
    const rows = records.map((r) => {
      ...
      const extraText = extraFieldDefs
        .map(
          (f) =>
            [
              f.label,
              formatFieldValueForDisplay(f, extra[f.key], {
                departmentLabel,
                employeeLabel,
              }),
            ] as const,
      ...
      return [
        ...
        deviceOwners.find((o) => o.code === r.device_owner)?.label ??
          r.device_owner ??
          "",
        r.department?.name ?? "",
        r.device_model ?? "",
  ...
  const validateImportRow = (raw: Record<string, string>) => {
    const preview = { ...raw };
    const errors: string[] = [];
    const type = raw.device_type
      ? findByLabel(deviceTypes, (t) => t.label, raw.device_type)
      : undefined;
    ...
    if (coreF.includes("device_owner")) {
      ...
      } else if (reqCoreF.includes("device_owner"))
        errors.push(`${label("device_owner")} is required`);
    }
    ...
    if (coreF.includes("department_id")) {
      ...
      } else if (reqCoreF.includes("department_id"))
        errors.push(`${label("department_id")} is required`);
    }
    if (
      coreF.includes("device_model") &&
      reqCoreF.includes("device_model") &&
      !raw.device_model
    )
      errors.push(`${label("device_model")} is required`);
    if (
      coreF.includes("hostname") &&
      reqCoreF.includes("hostname") &&
      !raw.hostname
    )
      errors.push(`${label("hostname")} is required`);
    ...
    if (
      baseF.includes("serial_number") &&
      reqBaseF.includes("serial_number") &&
      !raw.serial_number
    )
      errors.push(`${label("serial_number")} is required`);
    if (baseF.includes("mac_address")) {
      if (raw.mac_address && !isValidMac(raw.mac_address))
        errors.push(`${label("mac_address")} must look like 00:1A:2B:3C:4D:5E`);
      else if (reqBaseF.includes("mac_address") && !raw.mac_address)
        errors.push(`${label("mac_address")} is required`);
    }
    ...
    if (baseF.includes("location")) {
      ...
      } else if (reqBaseF.includes("location"))
        errors.push(`${label("location")} is required`);
    }
    if (
      baseF.includes("rack_number") &&
      reqBaseF.includes("rack_number") &&
      !raw.rack_number
    )
      errors.push(`${label("rack_number")} is required`);
    ...
    const values = {
      ...
      device_model: coreF.includes("device_model")
        ? raw.device_model || null
        : null,
      hostname: coreF.includes("hostname") ? raw.hostname || null : null,
      ip_address: baseF.includes("ip_address") ? ipValue : null,
      serial_number: baseF.includes("serial_number")
        ? raw.serial_number || null
        : null,
      mac_address: baseF.includes("mac_address")
        ? raw.mac_address || null
        : null,
      location: baseF.includes("location") ? locationValue : null,
      rack_number: baseF.includes("rack_number")
        ? raw.rack_number || null
        : null,
      model_id: null,
      image: null,
  ...
  const columns: Column<Device>[] = [
    ...
    },
    {
      ...
      render: (r) =>
        deviceTypes.find((t) => t.code === r.device_type)?.label ??
        r.device_type,
    },
    {
      key: "device_owner",
  ...
  const viewingType = viewing
    ? (deviceTypes.find((t) => t.code === viewing.device_type) ?? null)
    : null;
  ...
  const viewingFieldLabel = (key: string) =>
    viewingFieldLabels[key] ?? STD_FIELD_META[key]?.label ?? key;
    ...
    ? [
        {
          ...
          fields: [
            ...
            {
              label: "Device Type",
              value: viewingType?.label ?? viewing.device_type,
            },
            ...(viewingCoreFields.includes("device_owner")
              ? [
                  {
                    label: viewingFieldLabel("device_owner"),
                    value:
                      deviceOwners.find((o) => o.code === viewing.device_owner)
                        ?.label ?? viewing.device_owner,
                  },
                ]
              : []),
            ...(viewingCoreFields.includes("department_id")
              ? [
                  {
                    label: viewingFieldLabel("department_id"),
                    value: viewing.department?.name ?? null,
                  },
                ]
              : []),
            ...(viewingCoreFields.includes("device_model")
              ? [
                  {
                    label: viewingFieldLabel("device_model"),
                    value: viewing.device_model,
                  },
                ]
              : []),
            {
            ...
            ...(viewingCoreFields.includes("hostname")
              ? [
                  {
                    label: viewingFieldLabel("hostname"),
                    value: viewing.hostname,
                  },
                ]
              : []),
          ...
          ? [
              {
                ...
                fields: viewingExtraFields.map((f) => {
                  if (f.type === "image") {
                    ...
                    return {
                      label: f.label,
                      value: src ? <ZoomImage src={src} size={160} /> : null,
                    };
                  }
                  if (f.type === "file") {
                  ...
                  return {
                    ...
                    value: formatFieldValueForDisplay(
                      f,
                      viewingExtraData[f.key],
                      { departmentLabel, employeeLabel },
                    ),
                  };
                }),
              },
        ...
        viewingBaseFields.includes("mac_address")
          ? [
              {
                title: "Network",
                fields: [
                  ...
                  ...(viewingBaseFields.includes("mac_address")
                    ? [
                        {
                          label: viewingFieldLabel("mac_address"),
                          value: viewing.mac_address,
                          mono: true,
                        },
                      ]
                    : []),
          ...
          : []),
        ...(viewingBaseFields.includes("location") ||
        viewingBaseFields.includes("rack_number")
          ? [
              {
                title: "Location",
                fields: [
                  ...(viewingBaseFields.includes("location")
                    ? [
                        {
                          label: viewingFieldLabel("location"),
                  ...
                  ...(viewingBaseFields.includes("rack_number")
                    ? [
                        {
                          label: viewingFieldLabel("rack_number"),
                          value: viewing.rack_number,
                        },
                      ]
                    : []),
                ],
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {records.length} registered devices
            </p>
          </div>
        </div>
      ...
      <DetailsModal
        ...
        title={
          viewing
            ? (deviceTypes.find((t) => t.code === viewing.device_type)?.label ??
              viewing.hostname ??
              "Device")
            : ""
        }
      ...
      <Modal
        ...
        title={editing ? "Edit Device" : "Register New Device"}
        size="lg"
      >
        <form noValidate onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {coreFields.includes("device_owner") && (
              <Field
                label={fieldLabel("device_owner")}
                required={requiredCoreFields.includes("device_owner")}
                hint={
                  deviceOwners.length === 0
                    ? "No device owners defined yet - add one under Customization > Device Owners."
                    : undefined
                }
              >
                <SearchableSelect
                  options={deviceOwners.map((o) => ({
                    value: o.code,
                    label: o.label,
                  }))}
                ...
                {onNavigate && hasRole("admin") && (
                  ...
                  >
                    {deviceOwners.length === 0
                      ? "Add a device owner"
                      : "Don't see the right owner? Manage device owners"}
                  </button>
                )}
              </Field>
            )}
              ...
              <Field
                ...
                hint={
                  departments.length === 0
                    ? "No departments defined yet - add one under Customization > Departments."
                    : undefined
                }
              >
                <SearchableSelect
                ...
                {onNavigate && hasRole("admin") && (
                  ...
                  >
                    {departments.length === 0
                      ? "Add a department"
                      : "Don't see the right department? Manage departments"}
                  </button>
                )}
              </Field>
            )}
            ...
            {baseFields.includes("serial_number") && (
              <Field
                label={fieldLabel("serial_number")}
                required={requiredBaseFields.includes("serial_number")}
              >
                <TextInput
                  ...
                  placeholder={fieldPlaceholder("serial_number")}
                  required={requiredBaseFields.includes("serial_number")}
                />
              </Field>
            )}
            ...
            {baseFields.includes("rack_number") && (
              <Field
                label={fieldLabel("rack_number")}
                required={requiredBaseFields.includes("rack_number")}
              >
                <NumberInput
                  value={form.rack_number}
                  onChange={(e) =>
                    setForm({ ...form, rack_number: e.target.value })
                  }
                  placeholder={fieldPlaceholder("rack_number")}
                  required={requiredBaseFields.includes("rack_number")}
                />
              </Field>
            )}

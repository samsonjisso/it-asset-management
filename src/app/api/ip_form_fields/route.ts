...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const rows = await listRows(ipFormFieldsConfig, auth, req.nextUrl.searchParams);
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  const row = await createRow(ipFormFieldsConfig, auth, body as Record<string, unknown>);

...
import {
  requireModule as requireModuleAccess,
  requireRole as requireRoleAccess,
} from "@/server/middlewares/withAuth";
...
export function describeChanges(
  ...
  updateCols.forEach((col, i) => {
    ...
    const fmt = (v: any) =>
      booleanColumns.has(col) ? (v ? "Yes" : "No") : formatNotifyValue(v);
    changed.push(
      `${humanizeColumn(col, fieldLabels)}: ${fmt(before)} → ${fmt(after)}`,
    );
  });
  if (changed.length === 0) return null;
  const shown = changed.slice(0, 6);
  if (changed.length > shown.length)
    shown.push(`and ${changed.length - shown.length} more field(s)`);
  return shown.join("; ");
}
...
export async function listRows(
  ...
  const orderCol =
    orderParam && columns.includes(orderParam) ? orderParam : null;
  if (orderCol) {
  ...
  if (config.withDepartment)
    rows = await Promise.all(rows.map((r) => attachDepartment(r)));
  if (config.decorate)
    rows = await Promise.all(
      rows.map((r) => (r ? config.decorate!(r, pool as any) : r)),
    );
...
export async function createRow(
  ...
  const result = await withTransaction(async (conn) => {
    ...
    try {
      ...
      throw new ApiError(
        400,
        err?.sqlMessage || err?.message || "Insert failed",
      );
    }
...
export async function updateRow(
  ...
  const { json } = await withTransaction(async (conn) => {
    ...
    try {
      ...
      throw new ApiError(
        400,
        err?.sqlMessage || err?.message || "Update failed",
      );
    }

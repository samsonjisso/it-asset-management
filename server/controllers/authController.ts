...
function normalizePermissionsInput(value: unknown): string | null {
  ...
  const cleaned = [
    ...new Set(
      value.filter((v) => (MODULE_KEYS as readonly string[]).includes(v)),
    ),
  ];
...
export async function changeOwnPassword(
  ...
  if (current && bcrypt.compareSync(input.password, current.password_hash)) {
    throw new ApiError(
      400,
      "New password must be different from your current (or temporary) password",
    );
  }
...
export async function adminCreateUser(
  ...
  const complexityError = passwordComplexityError(input.password);
  if (complexityError) throw new ApiError(400, complexityError);
  if (input.role && !(ALL_ROLES as readonly string[]).includes(input.role))
...
export async function adminResetPassword(
  ...
  const forceChange = input.must_change_password === false ? 0 : 1;
  await pool.query(
    "UPDATE profiles SET password_hash = ?, must_change_password = ?, updated_at = ? WHERE id = ?",
    [hash, forceChange, nowSql(), targetId],
  );
  return { ok: true };
}

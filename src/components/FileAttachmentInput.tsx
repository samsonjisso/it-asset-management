...
import { FileText, X, Loader2, UploadCloud, FileImage, Download } from 'lucide-react';
...
export function FileAttachmentInput({ value, valueName, onChange, label = 'Attachment', hint }: FileAttachmentInputProps) {
  ...
  const handleFile = (file: File | undefined) => {
    setError('');
    if (!file) return;
    const typeOk = ACCEPTED_TYPES.includes(file.type) || /\.(pdf|docx?)$/i.test(file.name);
    if (!typeOk) {
      ...
      >
        {loading ? (
          ...
          <div className="flex items-center gap-3 p-3">
            {isImage ? (
              <img src={value} alt="" className="w-16 h-16 rounded-xl object-cover border border-brand-600 shrink-0" />
            ) : (
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-800 dark:text-gray-100 truncate">{valueName || 'Attached file'}</p>
          ...
          <div className="flex flex-col items-center gap-2 py-8 px-6 text-center">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1"><FileImage size={12} /> Image or PDF/Word document, up to 5MB</p>
          </div>
        )}
      </div>

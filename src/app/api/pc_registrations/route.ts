...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const rows = await listRows(pcRegistrationsConfig, auth, req.nextUrl.searchParams);
  return jsonOk(rows, { headers: NO_STORE_HEADERS });
});

export const POST = withErrorHandling(async (req: NextRequest) => {
  const auth = await requireAuth(req);
  const body = await parseBody(req, pcRegistrationSchema);
  const row = await createRow(pcRegistrationsConfig, auth, body as Record<string, unknown>);

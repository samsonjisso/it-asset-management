...
import { decodeMultiselect, encodeMultiselect, decodeFileValue, encodeFileValue, decodeCheckbox } from '../lib/deviceFieldValues';
...
export function DynamicField({ field, value, onChange, departments = [], employees = [] }: DynamicFieldProps) {
  const type = field.type ?? 'text';
  const options = field.options ?? [];

  switch (type) {
    case 'long_text':
      return <TextArea value={value} onChange={(e) => onChange(e.target.value)} placeholder={field.placeholder} required={field.required} rows={3} />;

    case 'number':
      return <NumberInput value={value} onChange={(e) => onChange(e.target.value)} placeholder={field.placeholder} required={field.required} />;
    ...
    case 'date':
      return <TextInput type="date" value={value} onChange={(e) => onChange(e.target.value)} required={field.required} />;

    case 'datetime':
      return <TextInput type="datetime-local" value={value} onChange={(e) => onChange(e.target.value)} required={field.required} />;
    ...
    case 'multiselect': {
      ...
      return (
        <div className="flex flex-wrap gap-2">
          {options.length === 0 && <span className="text-xs text-gray-400 dark:text-gray-500 italic">No options configured</span>}
          {options.map((opt) => (
            <label
              key={opt}
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs font-medium cursor-pointer select-none ${
                selected.includes(opt) ? 'border-brand-600 bg-brand-600 text-white' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 hover:border-brand-400'
              }`}
            >
    ...
    case 'radio':
      return (
        <div className="flex flex-wrap gap-3">
          {options.length === 0 && <span className="text-xs text-gray-400 dark:text-gray-500 italic">No options configured</span>}
          {options.map((opt) => (
            <label key={opt} className="inline-flex items-center gap-1.5 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
              <input
                type="radio"
                name={field.key}
                checked={value === opt}
    ...
    case 'email':
      return <TextInput type="email" value={value} onChange={(e) => onChange(e.target.value)} placeholder={field.placeholder || 'name@example.com'} required={field.required} />;

    case 'url':
      return <TextInput type="url" value={value} onChange={(e) => onChange(e.target.value)} placeholder={field.placeholder || 'https://...'} required={field.required} />;

    case 'image':
      return <ImageInput value={value || null} onChange={(dataUrl) => onChange(dataUrl || '')} label="" />;

    case 'file':
      return <FileInput value={decodeFileValue(value)} onChange={(f) => onChange(encodeFileValue(f))} />;
    ...
    case 'department':
      return (
        <SearchableSelect
          options={departments.filter((d) => !d.is_branch).map((d) => ({ value: d.id, label: d.name }))}
          value={value}
          onChange={onChange}
          placeholder={`Select ${field.label}`}
    ...
    case 'text':
    default:
      return <TextInput value={value} onChange={(e) => onChange(e.target.value)} placeholder={field.placeholder} required={field.required} />;
  }
}

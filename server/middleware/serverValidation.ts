...
import {
  assertSafeText,
  containsUnsafeScript,
  isValidHostname,
  isValidIpv4,
} from "./common.js";

/** Pure validation core - see pcValidation.ts for why this is split out. */
export function validateServerBody(body: Record<string, unknown>): void {
  const ip =
  ...
  const serverType =
    typeof body.server_type === "string" ? body.server_type.trim() : "";
  ...
  const owner =
    typeof body.server_owner === "string" ? body.server_owner.trim() : "";
  ...
  for (const key of [
    ...
    if (typeof value === "string") {
      if (containsUnsafeScript(value)) {
        throw new Error(
          `${key.replace(/_/g, " ")} contains invalid or unsafe script characters.`,
        );
      }
...
export function validateServer(
  ...
  try {
    ...
    validateServerBody((req.body ?? {}) as Record<string, unknown>);
    next();
  } catch (error) {

    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {subnets.length} subnet{subnets.length === 1 ? "" : "s"} defined —
              used to identify which network an IP address belongs to
            </p>
          </div>
        </div>
        {canManage && (
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-10 text-center text-gray-500 dark:text-gray-400">
          No subnets defined yet. Add one, e.g. prefix "10.6.13." labeled "Head
          Office - Server Room", and IP addresses starting with that prefix will
          be identified automatically on the Server Registration form.
        </div>
      ) : (
      ...
      >
        <form noValidate onSubmit={handleSave} className="space-y-4">
          ...
          <Field
            label="Label"
            required
            hint="What this subnet is / where it is"
          >
            <TextInput
              value={form.label}

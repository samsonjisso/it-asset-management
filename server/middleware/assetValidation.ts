...
export function validateAsset(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (assetName && containsUnsafeScript(assetName)) {
      throw new Error('Asset name contains invalid or unsafe script characters.');
    }

    const assetType = typeof body.asset_type === 'string' ? body.asset_type.trim() : '';
    if (assetType && containsUnsafeScript(assetType)) {
      throw new Error('Asset type contains invalid or unsafe script characters.');
    }
    ...
    for (const key of ['owner', 'location', 'model', 'serial_number', 'manufacturer', 'supplier', 'operating_system', 'notes']) {

...
export const UNSAFE_SCRIPT_RE =
  /<\s*(script|iframe|object|embed|svg|img)|javascript\s*:|vbscript\s*:|on\w+\s*=|<\s*\/?\s*[a-z]+\s*>/i;
...
export function assertSafeText(value: unknown, fieldName: string) {
  ...
  if (containsUnsafeScript(text)) {
    throw new Error(
      `${fieldName} contains invalid or unsafe script characters.`,
    );
  }
}

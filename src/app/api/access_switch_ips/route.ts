...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const rows = await listRows(accessSwitchIpsConfig, auth, req.nextUrl.searchParams);
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  const row = await createRow(accessSwitchIpsConfig, auth, body as Record<string, unknown>);

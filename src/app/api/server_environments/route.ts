...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const rows = await listRows(serverEnvironmentsConfig, auth, req.nextUrl.searchParams);
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  const row = await createRow(serverEnvironmentsConfig, auth, body as Record<string, unknown>);

...
export function AssetsPage() {
  ...
  const downloadTemplate = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      [
        "HQ Front Desk Laptop",
        "Laptop",
        departments[0]?.name ?? "IT Department",
        "Jane Doe",
        "3rd Floor",
        "Latitude 5420",
        "PC-HQ-010",
  ...
  function validateRow(
    ...
    const matchedType = assetTypeOptions.find(
    ...
    if (rawIp && !IPV4_RE.test(rawIp))
      errors.push(`Invalid IP Address "${rawIp}"`);
  ...
  const confirmImport = async () => {
    ...
    if (failed === 0) {
      toast(
        `Imported ${success} asset${success === 1 ? "" : "s"} successfully`,
        "success",
      );
    } else {
      toast(
        `Imported ${success} asset(s), ${failed} failed on save`,
        "warning",
      );
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-[#343494]">
              Asset Registration
            </h1>
            <p className="text-sm text-gray-500">
              {records.length} registered assets
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Asset Name" required>
              <TextInput
                ...
                placeholder="e.g., HQ Front Desk Laptop"
                required
              />
            </Field>
            ...
            <Field label="Owner">
              <TextInput
                ...
                placeholder="Employee or team responsible"
              />
            </Field>
            <Field label="Location">
              <TextInput
                ...
                placeholder="Datacenter, floor, or branch"
              />
            </Field>
            <Field label="Model">
              <TextInput
            ...
            <Field label="Hostname">
              <TextInput
                ...
                placeholder="Hostname (if applicable)"
              />
            </Field>
            ...
            <>
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between bg-[#f5f5fc] border border-gray-200 rounded-lg p-4">
                <div className="text-sm text-gray-600">
                  ...
                  <strong>.csv</strong> file. The first row must contain column
                  headers (Asset Name, Asset Type, Department, Owner, Location,
                  Model, Hostname, Serial Number, Manufacturer, Supplier,
                  Operating System, IP Address, Notes).
                </div>
                <Button
                  type="button"
                  variant="outline"
            ...
            <>
              <div className="flex items-start gap-3 bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <div className="text-sm text-gray-700">
                  ...
                  <p>
                    {importResult.success} asset(s) registered successfully.
                  </p>
                  {importResult.failed > 0 && (
                    <p className="text-amber-700">
                      {importResult.failed} row(s) failed while saving to the
                      server.
                    </p>
                  )}
                </div>
              ...
              <div className="flex justify-end pt-2 border-t border-gray-200">
                <Button type="button" variant="primary" onClick={closeImport}>
                  Done
                </Button>
              </div>
            </>
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm">
                ...
                {invalidImportRows.length > 0 && (
                  <span className="inline-flex items-center gap-1 text-red-700 bg-red-50 border border-red-200 rounded-full px-2.5 py-0.5">
                    <AlertTriangle size={14} /> {invalidImportRows.length} with
                    errors (won't be imported)
                  </span>
                )}
              </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {importRows.map((r) => (
                      <tr
                        key={r.rowNumber}
                        className={`border-t border-gray-100 ${r.errors.length ? "bg-red-50/60" : ""}`}
                      >
              ...
              <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {importing
                      ? "Importing..."
                      : `Import ${validImportRows.length} Asset${validImportRows.length === 1 ? "" : "s"}`}
                  </Button>
                </div>
              </div>
            </>

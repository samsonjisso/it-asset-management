...
export function validateDepartment(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const departmentName = typeof body.name === 'string' ? body.name.trim() : '';
    if (body.name !== undefined && body.name !== null && body.name !== '' && !departmentName) {
      throw new Error('Department name is required.');
    }
    if (departmentName && containsUnsafeScript(departmentName)) {
      throw new Error('Department name contains invalid or unsafe script characters.');
    }

    if (body.is_branch !== undefined && body.is_branch !== null && body.is_branch !== '' && typeof body.is_branch !== 'boolean') {

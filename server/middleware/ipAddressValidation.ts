...
import { assertSafeText, isNumeric, isValidHostname, isValidIpv4, isValidMacAddress } from './common.js';

export function validateIpAddress(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (hostname && !isValidHostname(hostname)) {
      throw new Error('Hostname must start with GBBIT01 and use only letters, numbers, or hyphens.');
    }
    ...
    const switchPort = typeof body.switch_port === 'string' ? body.switch_port.trim() : '';
    ...
    // Validate switch IP
    const switchIp = typeof body.switch_ip === 'string' ? body.switch_ip.trim() : '';
    ...
    for (const key of ['hostname', 'ip_owner', 'notes', 'switch_port', 'patch_panel_port', 'vlan']) {

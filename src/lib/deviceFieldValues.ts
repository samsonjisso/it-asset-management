...
import { isValidIPv4, isValidMac, isValidEmail, isValidUrl } from './validation';
...
export function decodeFileValue(raw: string | undefined): FileFieldValue | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === 'object' && typeof parsed.dataUrl === 'string') {
      return { name: typeof parsed.name === 'string' ? parsed.name : 'file', dataUrl: parsed.dataUrl };
    }
  } catch {
    // fall through
  }
...
export function isFieldValueFilled(field: DeviceTypeField, raw: string | undefined): boolean {
...
export function validateFieldValue(field: DeviceTypeField, raw: string | undefined): string | null {
  const type = field.type ?? 'text';
  ...
  switch (type) {
    case 'number':
      ...
      return null;
    case 'decimal':
      if (!/^-?\d+(\.\d+)?$/.test(value)) return `${field.label} must be a number`;
      return null;
    case 'ip_address':
      if (!isValidIPv4(value)) return `${field.label} must be a valid IPv4 address (e.g., 10.6.13.45)`;
      return null;
    case 'mac_address':
      if (!isValidMac(value)) return `${field.label} must look like 00:1A:2B:3C:4D:5E`;
      return null;
    case 'email':
      if (!isValidEmail(value)) return `${field.label} must be a valid email address`;
      return null;
    case 'url':
      if (!isValidUrl(value)) return `${field.label} must be a valid URL (starting with http:// or https://)`;
      return null;
    default:
      return null;
  }
...
export function formatFieldValueForDisplay(
  ...
  lookups?: { departmentLabel?: (code: string) => string; employeeLabel?: (id: string) => string }
): string {
  const type = field.type ?? 'text';
  ...
  switch (type) {
    ...
    case 'department':
    case 'branch':
      return (lookups?.departmentLabel ? lookups.departmentLabel(raw!.trim()) : raw!.trim()) || raw!.trim();
    case 'employee':
      return (lookups?.employeeLabel ? lookups.employeeLabel(raw!.trim()) : raw!.trim()) || raw!.trim();
    case 'date':

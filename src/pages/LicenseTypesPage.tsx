  ...
  return (
    <div className="space-y-4">
          ...
          {types.map((t) => (
            <div key={t.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-4 gbb-card-hover">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500 font-mono">{t.code}</p>
                  </div>
                </div>
                {canManage && (
                  <div className="flex gap-1">
                    <button onClick={() => openEditType(t)} className="p-1.5 text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/40 rounded-lg" title="Rename">
                      <Pencil size={16} />
                    </button>
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Subtypes</p>
                {(subtypesByType[t.id] ?? []).length === 0 ? (
                  <p className="text-xs text-gray-400 dark:text-gray-500 italic mb-2">No subtypes yet</p>
                ) : (
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {(subtypesByType[t.id] ?? []).map((s) => (
                      <span
                        key={s.id}
                        className="inline-flex items-center gap-1.5 pl-2.5 pr-1 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-medium"
                      >
                        {s.label}
                        {canManage && (
                          <button
      ...
      <Modal open={typeModalOpen} onClose={() => setTypeModalOpen(false)} title={editingType ? 'Rename License Type' : 'Add License Type'} size="sm">
        <form noValidate onSubmit={handleSaveType} className="space-y-4">

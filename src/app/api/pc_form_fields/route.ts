...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const rows = await listRows(pcFormFieldsConfig, auth, req.nextUrl.searchParams);
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  const row = await createRow(pcFormFieldsConfig, auth, body as Record<string, unknown>);

...
interface ImportModalProps {
  ...
  importRow: (values: Record<string, unknown>) => Promise<string | null | undefined>;
...
export function ImportModal({
  open, onClose, title, subtitle, columns, templateFilename, validateRow, importRow, onImported,
}: ImportModalProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const normalizedHeaders = headerCells.map((h) => normalizeHeader(h.replace(/\*$/, '')));
      ...
      const dataRows = aoa.slice(1).filter((r) => Array.isArray(r) && r.some((cell) => String(cell ?? '').trim() !== ''));
      const parsed: ParsedRow[] = dataRows.map((r, i) => {
        const raw: Record<string, string> = {};
        for (const col of columns) {
          const idx = colIndexByKey.get(col.key);
          raw[col.key] = idx !== undefined ? String((r as unknown[])[idx] ?? '').trim() : '';
        }
      ...
      setParseError('Could not read this file. Please upload a valid .xlsx, .xls, or .csv file exported from the template.');
      setRows([]);
    } finally {
      setParsing(false);
    }
  ...
  const runImport = async () => {
    ...
    for (let i = 0; i < next.length; i++) {
      if (next[i].error || !next[i].values) continue;
      const err = await importRow(next[i].values!);
      next[i] = err
        ? { ...next[i], status: 'failed', importError: err }
  ...
  return (
    <Modal open={open} onClose={handleClose} title={title} subtitle={subtitle} size="xl">
      <div className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3 bg-gray-50 dark:bg-gray-900 border border-brand-600 rounded-lg p-4">
          <div className="text-sm text-gray-600 dark:text-gray-300">
            Download the template, fill in your existing assets, then upload it here. Fields marked with <span className="font-semibold">*</span> are required.
          </div>
        ...
        <div>
          ...
          >
            <Upload size={22} />
            <span className="text-sm font-medium">{fileName ?? 'Click to upload a .xlsx, .xls, or .csv file'}</span>
            {!fileName && <span className="text-xs text-gray-400 dark:text-gray-500">Use the downloaded template for the expected columns</span>}
          </button>
        </div>
        ...
        {parseError && (
          <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg p-3">{parseError}</div>
        )}
        ...
        {!parsing && rows.length > 0 && (
          <>
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              {errorRows.length > 0 && (
                <span className="inline-flex items-center gap-1.5 text-red-600 font-medium">
                  <XCircle size={16} /> {errorRows.length} with errors (won't be imported)
                </span>
              )}
              {importDone && (
                <span className="inline-flex items-center gap-1.5 text-brand-700 dark:text-brand-300 font-medium">
                  <FileSpreadsheet size={16} /> Imported {importedCount}{failedCount > 0 ? `, ${failedCount} failed` : ''}
                </span>
              )}
            </div>
            ...
            <div className="border border-brand-600 rounded-lg overflow-auto max-h-80">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-900 sticky top-0">
                  <tr>
                    <th className="text-left px-3 py-2 font-medium text-gray-500 dark:text-gray-400 w-14">Row</th>
                    {columns.filter((c) => previewKeys.includes(c.key)).map((c) => (
                      <th key={c.key} className="text-left px-3 py-2 font-medium text-gray-500 dark:text-gray-400">{c.label}</th>
                    ))}
                ...
                <tbody className="divide-y divide-gray-100">
                  {rows.map((r) => (
                    <tr key={r.rowNum} className={r.error ? 'bg-red-50/50' : r.status === 'failed' ? 'bg-amber-50/50' : ''}>
                      <td className="px-3 py-2 text-gray-400 dark:text-gray-500">{r.rowNum}</td>
                      {columns.filter((c) => previewKeys.includes(c.key)).map((c) => (
                        <td key={c.key} className="px-3 py-2 text-gray-700 dark:text-gray-300 max-w-[10rem] truncate" title={r.preview[c.key]}>{r.preview[c.key] || '—'}</td>
                      ))}
                      <td className="px-3 py-2">
                        {r.status === 'imported' && <span className="text-green-700 inline-flex items-center gap-1"><CheckCircle2 size={14} /> Imported</span>}
                        {r.status === 'failed' && <span className="text-red-600 inline-flex items-center gap-1" title={r.importError}><XCircle size={14} /> {r.importError}</span>}
                        {r.status === 'pending' && r.error && <span className="text-red-600" title={r.error}>{r.error}</span>}
                        {r.status === 'pending' && !r.error && <span className="text-gray-400 dark:text-gray-500">Ready</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              >
                {importing ? (<><Loader2 size={16} className="animate-spin" /> Importing…</>) : `Import ${validRows.length} row${validRows.length === 1 ? '' : 's'}`}
              </Button>
            </div>
          </>
        )}

...
function drawHeader(ctx: PdfContext) {
  ...
  doc.text(`Generated: ${generatedAt.toLocaleString()}`, PAGE_W - MARGIN, 34, {
    align: "right",
  });
  if (periodLabel)
    doc.text(periodLabel, PAGE_W - MARGIN, 45, { align: "right" });
}
...
function drawFooter(doc: jsPDF, pageNum: number, totalPages: number) {
  ...
  doc.text(
    "Goh Betoch Bank - Asset Inventory Management Portal - Developed by Information Systems Department",
    MARGIN,
    PAGE_H - 24,
  );
  doc.text(`Page ${pageNum} of ${totalPages}`, PAGE_W - MARGIN, PAGE_H - 24, {
    align: "right",
  });
}
...
export async function generatePdfReport(opts: PdfReportOptions) {
  const { reportTitle, headers, rows, stats, periodLabel, generatedByName } =
    opts;
  ...
  const statItems: [string, number][] = [
    ...
    ["Network Devices", stats.totalNetworkDevices],
  ...
  const rightBottom = drawBarList(
    doc,
    rightX,
    rightTop,
    colW,
    stats.assetsByBranch,
    GOLD,
  );

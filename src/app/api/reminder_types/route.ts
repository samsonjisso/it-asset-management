...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const rows = await listRows(reminderTypesConfig, auth, req.nextUrl.searchParams);
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  const row = await createRow(reminderTypesConfig, auth, body as Record<string, unknown>);

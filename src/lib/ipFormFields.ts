...
export const IP_BASE_FIELD_META: Record<
  string,
...
export function parseIpBaseFields(config?: IpFormFields | null): string[] {
  ...
  return parsed.filter(
    (key: unknown): key is string =>
      typeof key === "string" && !!IP_BASE_FIELD_META[key],
  );
}

export function parseIpRequiredBaseFields(
  config?: IpFormFields | null,
): string[] {
  const parsed = parseJsonValue<unknown>(config?.required_base_fields);
  if (!Array.isArray(parsed)) return [];
  return parsed.filter(
    (key: unknown): key is string =>
      typeof key === "string" && ALL_IP_BASE_FIELDS.includes(key),
  );
}

export function parseIpFieldLabels(
  config?: IpFormFields | null,
...
export function parseIpExtraFields(
  config?: IpFormFields | null,
): DeviceTypeField[] {

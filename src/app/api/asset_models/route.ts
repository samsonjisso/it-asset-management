...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const rows = await listRows(assetModelsConfig, auth, req.nextUrl.searchParams);
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  const row = await createRow(assetModelsConfig, auth, body as Record<string, unknown>);

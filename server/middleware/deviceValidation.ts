...
export function validateDeviceBody(body: Record<string, unknown>): void {
  const validOwners = [
  ...
  const owner =
    typeof body.device_owner === "string" ? body.device_owner.trim() : "";
...
export function validateDevice(
  ...
  try {
    ...
    validateDeviceBody((req.body ?? {}) as Record<string, unknown>);
    next();
  } catch (error) {

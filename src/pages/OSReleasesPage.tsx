    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {releases.length} OS release{releases.length === 1 ? "" : "s"}{" "}
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-10 text-center text-gray-500 dark:text-gray-400">
          No OS releases yet. Add one to make it available on the Server
          Registration form.
        </div>
      ) : (
      ...
      <Modal
        ...
        title={editing ? "Rename OS Release" : "Add OS Release"}
        size="sm"
      >
        <form noValidate onSubmit={handleSave} className="space-y-4">
          <Field
            label="Name"
            required
            hint="e.g., Red Hat Enterprise Linux 9, Windows Server 2022, Ubuntu Server"
          >
            <TextInput
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              placeholder="e.g., Red Hat Enterprise Linux 9"
              required
              autoFocus
            />
          </Field>

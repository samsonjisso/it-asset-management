...
import { assetModelsConfig } from '@/server/controllers/crudConfig';
...
export const GET = withErrorHandling(async (req: NextRequest, { params }: { params: { id: string } }) => {
  ...
  const row = await getRowById(assetModelsConfig, auth, params.id);
...
export const PATCH = withErrorHandling(async (req: NextRequest, { params }: { params: { id: string } }) => {
  ...
  const row = await updateRow(assetModelsConfig, auth, params.id, body as Record<string, unknown>);
  return jsonOk(row, { headers: NO_STORE_HEADERS });
});

export const DELETE = withErrorHandling(async (req: NextRequest, { params }: { params: { id: string } }) => {
  const auth = await requireAuth(req);
  await deleteRow(assetModelsConfig, auth, params.id);

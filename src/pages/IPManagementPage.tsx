...
const statusStyles: Record<IPAddress["status"], string> = {
  unassigned:
    "bg-gray-50 dark:bg-gray-900 text-gray-400 dark:text-gray-500 ring-1 ring-gray-200",
  assigned: "bg-green-50 text-green-700 ring-1 ring-green-100",
  reserved: "bg-amber-50 text-amber-700 ring-1 ring-amber-100",
  available:
    "bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 ring-1 ring-brand-100",
  decommissioned:
    "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 ring-1 ring-gray-200",
};
...
export function IPManagementPage({
  ...
  const [boardFilter, setBoardFilter] = useState<"all" | "used" | "available">(
    "all",
  );
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const [ipRes, deptRes, subnetRes, patchPanelRes, employeesRes, fieldsRes] =
      await Promise.all([
        supabase
          .from("ip_addresses")
    ...
    if (patchPanelRes.data)
      setPatchPanelLabels(patchPanelRes.data as PatchLevel[]);
    if (employeesRes.data) setEmployees(employeesRes.data as DirectoryUser[]);
    if (fieldsRes.data)
      setIpFormConfig((fieldsRes.data as IpFormFields[])[0] ?? null);
    setLoading(false);
  }, []);
  ...
  const baseFields = useMemo(
    () => parseIpBaseFields(ipFormConfig),
    [ipFormConfig],
  );
  const requiredBaseFields = useMemo(
    () => parseIpRequiredBaseFields(ipFormConfig),
    [ipFormConfig],
  );
  const fieldLabels = useMemo(
    () => parseIpFieldLabels(ipFormConfig),
    [ipFormConfig],
  );
  const extraFields = useMemo(
    () => parseIpExtraFields(ipFormConfig),
    [ipFormConfig],
  );
  ...
  const openEdit = (rec: IPAddress) => {
    ...
    if (rec.extra_data) {
      try {
        const parsed =
          typeof rec.extra_data === "string"
            ? JSON.parse(rec.extra_data)
            : rec.extra_data;
        if (parsed && typeof parsed === "object")
          extraData = parsed as Record<string, string>;
      } catch {
        extraData = {};
      }
    }
    setForm({
      subnet_id:
        rec.subnet_id ?? matchSubnet(rec.ip_address, subnets)?.id ?? "",
  ...
  const handleSave = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    const requiredFields: [keyof typeof form, string][] = [
      ...
      ...requiredBaseFields.map(
        (key) =>
          [key as keyof typeof form, fieldLabel(key)] as [
            keyof typeof form,
            string,
          ],
      ),
    ...
    if (!form.ip_address.startsWith(chosenSubnet.prefix.trim())) {
      toast(
        `IP Address does not belong to the selected subnet (${chosenSubnet.prefix}).`,
        "error",
      );
      return;
    }
    ...
    if (conflict && conflict.id !== editing?.id) {
      toast(
        "This IP Address is already registered/assigned. Choose a different available address.",
        "error",
      );
      return;
    }
    ...
    const hostnameConflict = form.hostname
      ? findHostnameConflict(form.hostname)
      : null;
    if (hostnameConflict) {
      if (!editing) {
        toast(
          `Hostname "${form.hostname.trim()}" is already registered. Continue in PC Registration to register this device.`,
          "error",
        );
        router.push(
          `/pc?create=${Date.now()}&hostname=${encodeURIComponent(form.hostname.trim())}&ip_address=${encodeURIComponent(form.ip_address.trim())}`,
        );
        return;
      }
      toast(
    ...
    if (form.access_switch_port && !isValidPortLabel(form.access_switch_port)) {
      toast(
        "Access Switch Port / Interface Number contains invalid characters",
        "error",
      );
      return;
    }
  ...
  const exportCSV = () => {
    ...
    const rows = records.map((r) => {
      const s =
        subnets.find((sn) => sn.id === r.subnet_id) ??
        matchSubnet(r.ip_address, subnets);
      return [
        r.ip_address,
        s ? `${s.prefix}0/24` : "",
        r.hostname ?? "",
  ...
  const importColumns: ImportColumn[] = [
    ...
    {
      key: "access_switch_port",
      label: "Access Switch Port / Interface Number",
      required: true,
      example: "Gi0/1",
    },
    {
      key: "patch_panel_label",
      label: "Patch Panel Label / Number",
      required: true,
      example: patchPanelLabels[0]?.label,
    },
    {
      key: "status",
      label: "Status (Assigned/Reserved/Available/Decommissioned)",
      required: true,
      example: "Assigned",
    },
  ...
  const validateImportRow = (raw: Record<string, string>) => {
    ...
    let subnet = raw.subnet
      ? subnets.find((s) => s.prefix.trim() === raw.subnet?.trim())
      : undefined;
    if (!subnet && ip && isValidIPv4(ip))
      subnet = matchSubnet(ip, subnets) ?? undefined;
    if (!subnet)
      errors.push(
        raw.subnet
          ? `Subnet "${raw.subnet}" not found`
          : `No configured subnet matches ${ip || "this IP"} — add one under Customization > IP Subnets`,
      );
    else if (ip && isValidIPv4(ip) && !ip.startsWith(subnet.prefix.trim()))
      errors.push(`IP Address does not belong to subnet ${subnet.prefix}`);

    if (ip && isValidIPv4(ip)) {
      const existing = recordByIp.get(ip);
      if (existing) errors.push(`IP Address ${ip} is already registered`);
      else if (importedIpsInBatch.has(ip))
        errors.push(`IP Address ${ip} is duplicated elsewhere in this file`);
    }
    ...
    else if (!isValidEmployeeName(raw.ip_owner))
      errors.push("IP Address Owner contains invalid characters");

    if (!raw.mac_address) errors.push("MAC Address is required");
    else if (!isValidMac(raw.mac_address))
      errors.push("MAC Address must look like 00:1A:2B:3C:4D:5E");

    if (!raw.hostname) errors.push("Hostname is required");
    else if (!isValidHostname(raw.hostname))
      errors.push(
        "Hostname may only contain letters, numbers, hyphens and dots",
      );
    else {
      const hLower = raw.hostname.trim().toLowerCase();
      if (findHostnameConflict(raw.hostname))
        errors.push(
          `Hostname "${raw.hostname.trim()}" is already registered to another active record`,
        );
      else if (importedHostnamesInBatch.has(hLower))
        errors.push(
          `Hostname "${raw.hostname.trim()}" is duplicated elsewhere in this file`,
        );
    }

    if (!raw.access_switch_port)
      errors.push("Access Switch Port / Interface Number is required");
    else if (!isValidPortLabel(raw.access_switch_port))
      errors.push(
        "Access Switch Port / Interface Number contains invalid characters",
      );

    let patchLabel: string | null = null;
    if (!raw.patch_panel_label)
      errors.push("Patch Panel Label / Number is required");
    else {
      const pl = findByLabel(
        patchPanelLabels,
        (p) => p.label,
        raw.patch_panel_label,
      );
      if (!pl)
        errors.push(`Patch Panel Label "${raw.patch_panel_label}" not found`);
    ...
    if (!raw.status) errors.push("Status is required");
      ...
      const match = statusOptions.find(
        (s) =>
          s.label.toLowerCase() === raw.status?.trim().toLowerCase() ||
          s.value === raw.status?.trim().toLowerCase(),
      );
      if (!match || match.value === "unassigned")
        errors.push(
          `Status must be one of Assigned, Reserved, Available, or Decommissioned`,
        );
    ...
    if (raw.hostname)
      importedHostnamesInBatch.add(raw.hostname.trim().toLowerCase());
  ...
  const ipSearchValue = (r: IPAddress) => {
    const subnet =
      subnets.find((s) => s.id === r.subnet_id) ??
      matchSubnet(r.ip_address, subnets);
    return [
      r.ip_address,
      r.hostname,
  ...
  const columns: Column<IPAddress>[] = [
    ...
    { key: "ip_owner", label: "Owner", render: (r) => r.ip_owner ?? "-" },
    {
      ...
      render: (r) => (
        <span
          className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${statusStyles[r.status]}`}
        >
          {r.status}
        </span>
      ),
    ...
    },
    {
      ...
      render: (r) => (
        <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={() => openView(r)}
            className="p-1.5 text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/40 rounded-lg transition-colors"
            title="View Details"
          >
            <Eye size={16} />
          </button>
  ...
  const formSubnetEnumerable =
    !!formSubnet && isEnumerableSubnet(formSubnet.prefix);
  ...
  const selectedBoardSubnet =
    subnets.find((s) => s.id === boardSubnetId) ?? null;
  const boardSubnetEnumerable =
    !!selectedBoardSubnet && isEnumerableSubnet(selectedBoardSubnet.prefix);

  type BoardCell = {
    ip: string;
    octet: number;
    kind: "network" | "used" | "free";
  ...
  const boardCellStyles: Record<BoardCell["kind"], string> = {
    network:
      "bg-gray-50 dark:bg-gray-900 text-gray-300 dark:text-gray-600 ring-1 ring-gray-100 cursor-not-allowed",
    ...
    ? [
        {
          ...
          fields: [
            { label: "IP Address", value: viewing.ip_address, mono: true },
            {
              ...
              value: (() => {
                const s =
                  subnets.find((sn) => sn.id === viewing.subnet_id) ??
                  matchSubnet(viewing.ip_address, subnets);
                return s
                  ? `${s.prefix}0/24${s.label ? ` — ${s.label}` : ""}`
                  : undefined;
              })(),
            },
            ...
            {
              label: "Status",
              value: statusOptions.find((s) => s.value === viewing.status)
                ?.label,
            },
        ...
        },
        {
          ...
          fields: [
            {
              label: "Access Switch Port / Interface Number",
              value: viewing.access_switch_port,
            },
            {
              label: "Patch Panel Label / Number",
              value: viewing.patch_panel_label,
            },
          ],
        },
        {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {statusCounts.map((s) => {
          ...
          return (
            <div
              key={s.value}
              className="gbb-card-hover bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-4"
            >
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`w-11 h-11 rounded-xl bg-gradient-to-br ${style.color} text-white flex items-center justify-center shadow-soft shrink-0`}
                >
                  {style.icon}
                </div>
              </div>
      ...
      >
        <form noValidate onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              <SearchableSelect
                options={usableSubnets.map((s) => ({
                  value: s.id,
                  label: `${s.prefix}0/24${s.label ? ` — ${s.label}` : ""}`,
                }))}
                value={form.subnet_id}
                onChange={(val) =>
                  setForm({ ...form, subnet_id: val, ip_address: "" })
                }
            ...
            >
              <SearchableSelect
                options={formAvailableIps.map((ip) => ({
                  value: ip,
                  label: `${ip}${ip === editing?.ip_address ? " (current)" : ""}`,
                }))}
            ...
            <Field
              ...
              hint={
                hostnameOptions.length > 0
                  ? "Start typing to see hostnames already in the system"
                  : undefined
              }
            >
              <SearchableCombobox
            ...
            <Field
              ...
              hint={
                ipOwnerOptions.length > 0
                  ? "Start typing to see owners already in the system"
                  : undefined
              }
            >
              <SearchableCombobox
            ...
            <Field
              ...
              hint={
                macOptions.length > 0
                  ? "Start typing to see MAC addresses already in the system"
                  : undefined
              }
            >
              <SearchableCombobox
            ...
            <Field
              label={fieldLabel("access_switch_port")}
              required={requiredBaseFields.includes("access_switch_port")}
            >
              <TextInput
                value={form.access_switch_port}
                onChange={(e) =>
                  setForm({ ...form, access_switch_port: e.target.value })
                }
            ...
            >
              <SearchableSelect
                options={patchPanelLabels.map((l) => ({
                  value: l.label,
                  label: l.label,
                }))}
                ...
                emptyMessage={
                  patchPanelLabels.length === 0
                    ? "No patch panel labels defined yet."
                    : "No matching labels."
                }
                required
              />
            ...
            <Field
              ...
              hint={
                form.status === "unassigned"
                  ? "Select a status to complete registration"
                  : undefined
              }
            >
              <SelectInput
          ...
          <Field
            ...
            required={requiredBaseFields.includes("notes")}
          >
            <TextArea
              value={form.notes}
              ...
              rows={2}
              placeholder={
                IP_BASE_FIELD_META.notes?.placeholder ?? "Additional notes..."
              }
              required={requiredBaseFields.includes("notes")}
            />
          </Field>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-3 items-end">
            <Field
              ...
              hint={
                subnets.length === 0
                  ? "No subnets defined yet — add one under IP Subnet Management"
                  : undefined
              }
            >
              <SearchableSelect
            ...
            {boardSubnetEnumerable && (
              <div
                className="flex rounded-lg ring-1 ring-gray-200 overflow-hidden shrink-0"
                role="group"
                aria-label="IP usage filter"
              >
                {(
                  [
                    { value: "all", label: "All" },
                  ...
                  <button
                    ...
                    className={`px-3 py-2 text-xs font-semibold transition-colors ${
                      boardFilter === f.value
                        ...
                        : "bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900"
                    }`}
                  >
                    {f.label}
                  </button>
          ...
          {selectedBoardSubnet && !boardSubnetEnumerable && (
            <div className="rounded-xl bg-amber-50 ring-1 ring-amber-100 p-4 text-sm text-amber-700">
              This subnet's prefix ("{selectedBoardSubnet.prefix}") describes
              more than 256 addresses, so it can't be drawn as a full board.
              Define it down to three octets (e.g. "10.6.13.") to enable the
              board view.
            </div>
          )}
            ...
            <>
              <div className="flex items-center gap-4 flex-wrap text-sm">
                <span className="flex items-center gap-1.5 text-gray-500 dark:text-gray-400">
                  ...
                  {selectedBoardSubnet.prefix}0 – {selectedBoardSubnet.prefix}
                  255
                </span>
              ...
              <div className="grid grid-cols-[repeat(8,minmax(0,1fr))] sm:grid-cols-[repeat(16,minmax(0,1fr))] gap-1.5">
                {boardCells.map((cell) => {
                  const dimmed =
                    boardFilter !== "all" &&
                    cell.kind !== "network" &&
                    ((boardFilter === "used" && cell.kind !== "used") ||
                      (boardFilter === "available" && cell.kind !== "free"));
                  return (
                    <button
                      key={cell.ip}
                      type="button"
                      ...
                      onClick={() => {
                        if (cell.kind === "used" && cell.record)
                          openViewFromBoard(cell.record);
                        else if (cell.kind === "free")
                          openAddFromBoard(cell.ip);
                      }}
                      title={
                        cell.kind === "network"
              ...
              <div className="flex items-center gap-4 flex-wrap text-xs text-gray-400 dark:text-gray-500 pt-1">
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-red-50 ring-1 ring-red-100 inline-block" />{" "}
                  Used / assigned
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-green-50 ring-1 ring-green-100 inline-block" />{" "}
                  Available
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-gray-50 dark:bg-gray-900 ring-1 ring-gray-100 inline-block" />{" "}
                  Network / broadcast
                </span>
              </div>
                      ...
                      {visibleBoardCells
                        ...
                        .map((cell) => (
                          <tr
                            ...
                            onClick={() =>
                              cell.kind === "used" && cell.record
                                ? openViewFromBoard(cell.record)
                                : openAddFromBoard(cell.ip)
                            }
                          >
                            <td className="px-3 py-1.5 font-mono text-xs font-semibold text-brand-700 dark:text-brand-300">
                              {cell.ip}
                            </td>
                            <td className="px-3 py-1.5">
                              <span
                                className={`text-[11px] font-semibold px-2 py-0.5 rounded-full capitalize ${
                                  cell.record
                                    ? statusStyles[cell.record.status]
          ...
          <div className="pt-4 border-t border-gray-100 dark:border-gray-800 space-y-3">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">
              Or ping a single address directly
            </p>
            ...
            {!checking && checkResult && (
              ...
              >
                ...
                <div>
                  <p
                    className={`text-sm font-semibold ${checkResult.reachable ? "text-red-700" : "text-green-700"}`}
                  >
                    {checkResult.message}
                  </p>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">
              A live ICMP ping — a host can be offline yet still reserved, so
              confirm against the board above before assigning.
            </p>
          </div>
        </div>
      </Modal>

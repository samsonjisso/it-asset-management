...
export function PatchLevelsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast(
        editing ? "Patch/level value updated" : "Patch/level value added",
        "success",
      );
      setModalOpen(false);
      loadData();
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-brand-600">
              Patch / Level Number Management
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {levels.length} value{levels.length === 1 ? "" : "s"} available on
              the PC Registration and IP Management forms
            </p>
          </div>
        </div>
        {canManage && (
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-10 text-center text-gray-500 dark:text-gray-400">
          No patch/level values yet. Add one to make it available on the PC
          Registration form.
        </div>
      ) : (
      ...
      <Modal
        ...
        title={editing ? "Rename Patch/Level Value" : "Add Patch/Level Value"}
        size="sm"
      >

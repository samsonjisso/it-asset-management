...
export function ProfilePage() {
  ...
  const roleLabels: Record<string, string> = {
    admin: 'Administrator', editor: 'Editor', reader: 'Reader (Read Only)', audit: 'Audit (Read Only)',
  };

  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-brand-600 to-brand-400 text-white flex items-center justify-center shadow-soft"><User size={22} /></div>
        <div>
          <h1 className="text-xl font-bold text-brand-600">My Profile</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Update your information and change password</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-6">
        <div className="flex items-center gap-4 mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{profile?.full_name}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1"><Mail size={14} />{profile?.email}</p>
        ...
        <form noValidate onSubmit={handleUpdateInfo} className="space-y-4">
          <h4 className="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2"><User size={18} /> Personal Information</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field
              label="Email"
              required={isAdmin}
              hint={isAdmin ? undefined : 'Contact an administrator to change your email address.'}
            >
              {isAdmin ? (
                <TextInput type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
              ) : (
                <TextInput value={profile?.email ?? ''} disabled className="bg-gray-50 dark:bg-gray-900" />
              )}
            </Field>
            ...
            <Field
              label="Role"
              hint={isAdmin ? "Admins can't change their own role — ask another admin if this needs updating." : 'Set by an administrator.'}
            >
              {isAdmin ? (
                ...
                <TextInput value={roleLabels[profile?.role ?? '']} disabled className="bg-gray-50 dark:bg-gray-900" />
              )}
            </Field>
          </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-6">
        <form noValidate onSubmit={handleChangePassword} className="space-y-4">
          <h4 className="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2"><KeyRound size={18} /> Change Password</h4>
          <p className="text-xs text-gray-500 dark:text-gray-400 -mt-2">
            At least 8 characters, with upper/lowercase letters, a number, and a symbol. It can't be the same as your current password.
          </p>
          <Field label="New Password" required>
            <div className="relative">
              <Lock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500" />
              <TextInput type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="New password" className="pl-10" required minLength={8} />
            </div>
          </Field>
          <Field label="Confirm New Password" required>
            <div className="relative">
              <Lock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500" />
              <TextInput type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm new password" className="pl-10" required minLength={8} />
            </div>
          </Field>
          <div className="flex justify-end">

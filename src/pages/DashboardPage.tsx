...
export function DashboardPage({
  ...
  useEffect(() => {
    (async () => {
      ...
      servers.forEach((s) => {
        byEnv[s.environment] = (byEnv[s.environment] ?? 0) + 1;
      });
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const count = [...pcs, ...licenses, ...devices, ...servers].filter(
          (r) => {
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-xl px-4 py-2.5 self-start sm:self-auto ring-1 ring-white/10">
            ...
            <span className="text-xs font-medium text-white/90">
              {pcCount + ipCount + licenseCount + deviceCount + serverCount}{" "}
              assets tracked
            </span>
          </div>
        </div>
          ...
          return (
            <button
              ...
              className={`text-left w-full bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-5 gbb-card-hover transition-shadow ${
                clickable
                  ? "cursor-pointer hover:shadow-lift hover:border-brand-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
                  : "cursor-default"
              }`}
            >
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`w-12 h-12 rounded-xl bg-gradient-to-br ${stat.color} text-white flex items-center justify-center shadow-soft`}
                >
                  {stat.icon}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto text-xs px-2 py-1 rounded-full bg-amber-50 text-amber-700 font-semibold">
              ...
              {expiringLicenses.map((l) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-semibold px-2 py-1 rounded-full ${days < 0 ? "text-red-600 bg-red-50" : "text-amber-700 bg-amber-100"}`}
                    >
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto text-xs px-2 py-1 rounded-full bg-brand-50 dark:bg-brand-900/40 text-brand-600 font-semibold">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {upcomingReminders.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className="flex items-center justify-between bg-brand-50/50 rounded-lg px-3 py-2"
                  >
                    <div>
                      <p className="text-sm font-medium text-gray-800 dark:text-gray-100">
                        {r.title}
                    ...
                    <span
                      className={`text-xs font-semibold px-2 py-1 rounded-full ${days <= 0 ? "text-red-600 bg-red-50" : "text-brand-700 dark:text-brand-300 bg-brand-100 dark:bg-brand-900/50"}`}
                    >
                      {days <= 0 ? "Due!" : `${days}d`}
                    </span>
                  </div>
            ...
            {monthlyData.map((m, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                ...
                >
                  ...
                  >
                    <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-xs font-semibold text-gray-700 dark:text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity">
                      {m.count}
                    </span>
                  </div>
                </div>
          ...
          <div className="space-y-3">
            {["production", "test", "standby"].map((env) => {
              ...
              const colors = {
                ...
                test: "bg-brand-500",
                standby: "bg-gray-400 dark:bg-gray-600",
              };
              return (
                <div key={env}>
                  ...
                  <div className="w-full h-3 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${colors[env as keyof typeof colors]} rounded-full transition-all duration-500`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-5">
        ...
        {Object.keys(deviceByType).length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {Object.entries(deviceByType)
              .sort((a, b) => b[1] - a[1])
              .map(([type, count]) => (
                <div
                  key={type}
                  className="flex items-center justify-between bg-gray-50 dark:bg-gray-900 rounded-lg px-3 py-2.5"
                >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {pc.ip_address ?? "No IP"} - {pc.service_tag ?? "No tag"}
                    </p>
                  </div>
                </div>

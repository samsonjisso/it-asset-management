...
const emptyForm = {
  ...
  mac_address: "",
  license_id: "",
  // Temporary manual value; the server registers it as a license record
  // and links it to this PC when the form is saved.
  product_key: "",
  cpu: "",
  memory_detail: "",
  generation_detail: "",
...
export function PCRegistrationPage({
  ...
  const handleSave = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    for (const key of requiredBaseFields) {
      // Asset Tag / ID is generated by the backend when left blank.
    ...
    if (
      baseFields.includes("memory_detail") &&
      form.memory_detail.trim() &&
      !isValidRam(form.memory_detail)
    ) {
      toast(`${fieldLabel("memory_detail")} must be a positive number in GB`, "error");
      return;
    }
  ...
  const validateImportRow = (raw: Record<string, string>) => {
    ...
    if (raw.memory_detail && !isValidRam(raw.memory_detail))
      errors.push(`${fieldLabel("memory_detail")} must be a positive number in GB`);
  ...
  const renderBaseField = (key: string): ReactNode => {
    ...
    switch (key) {
      ...
      case "asset_tag":
        // The backend generates the asset ID/tag when this is blank.
        return (
          <Field
            key={key}
            label="Asset Tag / ID"
            required
            hint="Leave blank to generate the next GBB-COMP asset ID"
          >
            <TextInput
      ...
      case "memory_detail":
        return (
          <Field key={key} label={`${fieldLabel(key)} (GB)`} required={required}>
            <TextInput
              type="number"
              min={0.01}
              step="any"

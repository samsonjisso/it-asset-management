...
import { Plus, Pencil, Users, UserCheck, UserX, Mail, KeyRound, ShieldAlert, ShieldCheck, Trash2, Crown } from 'lucide-react';

const roleOptions: { value: UserRole; label: string; hint: string }[] = [
  { value: 'admin', label: 'Admin', hint: 'Full system access and management' },
  { value: 'editor', label: 'Editor', hint: 'Can add and modify asset information' },
  { value: 'reader', label: 'Reader', hint: 'View-only access' },
  { value: 'audit', label: 'Audit', hint: 'Can view everything but cannot edit or delete' },
];

const roleColors: Record<string, string> = {
  admin: 'text-red-700 bg-red-50 border-red-200',
  editor: 'text-brand-700 dark:text-brand-300 bg-brand-50 dark:bg-brand-900/40 border-brand-200',
  reader: 'text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700',
  audit: 'text-purple-700 bg-purple-50 border-purple-200',
};

const roleLabels: Record<string, string> = {
  admin: 'Admin', editor: 'Editor', reader: 'Reader', audit: 'Audit',
};
...
function ModuleAccessChecklist({
  ...
  const renderGroup = (label: string, mods: typeof available) => (
    mods.length > 0 && (
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500 mb-1.5">{label}</p>
  ...
  return (
    <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
      ...
      {available.length === 0 && (
        <p className="text-xs text-gray-400 dark:text-gray-500">No restrictable modules for this role.</p>
      )}
    </div>
  );
}
...
export function UserManagementPage() {
  ...
  const [newUser, setNewUser] = useState({
    email: '', full_name: '', password: '', role: 'reader' as UserRole, phone: '', must_change_password: true,
    restrictAccess: false, permissions: [] as string[],
  });
  const [editForm, setEditForm] = useState({
    full_name: '', email: '', role: 'reader' as UserRole, phone: '', is_active: true, must_change_password: false,
    restrictAccess: false, permissions: [] as string[],
  });
  const [resetForm, setResetForm] = useState({ password: '', force_change: true });
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (newUser.restrictAccess && newUser.permissions.length === 0) {
      toast('Tick at least one module, or turn off access restriction', 'error');
      return;
    }
    setSaving(true);
    ...
    setModalOpen(false);
    setNewUser({
      email: '', full_name: '', password: '', role: 'reader', phone: '', must_change_password: true,
      restrictAccess: false, permissions: [],
    });
    loadData();
  };
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('profiles').update({
      ...
      ...(isSelf ? {} : { permissions: editForm.restrictAccess ? editForm.permissions : null }),
  ...
  const handleTransferOwnership = async (e: React.FormEvent) => {
    ...
    const { error } = await transferOwnership(transferTarget.id, transferPassword);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-brand-600 to-brand-400 text-white flex items-center justify-center shadow-soft"><Users size={22} /></div>
          <div>
            <h1 className="text-xl font-bold text-brand-600">User Management</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">{users.length} users registered</p>
          </div>
        </div>
        {isAdmin && <Button variant="primary" size="sm" onClick={() => setModalOpen(true)}><Plus size={16} /> Create User</Button>}
      </div>

      {!isAdmin && (
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 text-xs text-purple-700 flex items-center gap-2">
          <ShieldAlert size={16} /> You have read-only (Audit) access to user accounts. Only Admins can create, edit, or reset users.
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {users.map((user) => (
            <div key={user.id} className={`bg-white dark:bg-gray-900 rounded-2xl shadow-card border border-brand-600 p-4 gbb-card-hover ${!user.is_active ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1"><Mail size={11} />{user.email}</p>
                  </div>
                </div>
                {user.id === profile?.id && <span className="text-xs text-gold-400 font-medium">You</span>}
              </div>
              <div className="mt-3 flex items-center justify-between flex-wrap gap-1.5">
                ...
                <span className={`text-xs ${user.is_active ? 'text-green-600' : 'text-gray-400 dark:text-gray-500'}`}>{user.is_active ? 'Active' : 'Disabled'}</span>
              </div>
              {user.is_owner && (
                <div className="mt-2 flex items-center gap-1.5 text-[11px] text-red-700 bg-red-50 border border-red-200 rounded-lg px-2 py-1">
                  <Crown size={12} /> Owner — cannot be deleted, demoted, or disabled by other admins
                </div>
              )}
              ...
              {user.permissions && user.permissions.length > 0 && (
                <div className="mt-2 text-[11px] text-teal-700 bg-teal-50 border border-teal-200 rounded-lg px-2 py-1.5">
                  <div className="flex items-center gap-1.5 font-medium mb-1"><ShieldCheck size={12} /> Restricted access</div>
                  <p className="text-teal-600 leading-snug">
                    {user.permissions.map((m) => MODULE_LABELS[m] ?? m).join(', ')}
                  </p>
                </div>
              )}
              {isAdmin && (
                <div className="mt-3 flex flex-wrap gap-2 pt-3 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => openReset(user)} className="flex-1"><KeyRound size={14} /> Reset Password</Button>
                  ...
                  {profile?.is_owner && user.id !== profile?.id && user.role === 'admin' && user.is_active && (
                    <Button variant="ghost" size="sm" onClick={() => openTransfer(user)} className="flex-1">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Create New User" size="md">
        <form noValidate onSubmit={handleCreateUser} className="space-y-4">
          ...
          <Field label="Temporary Password" required hint="At least 8 characters, with upper/lowercase, a number, and a symbol. The user will be asked to set their own password if 'force change' is checked below.">
            <TextInput type="password" value={newUser.password} onChange={(e) => setNewUser({ ...newUser, password: e.target.value })} placeholder="Temporary password" required minLength={8} />
          </Field>
          <Field label="Role" required>
            <SelectInput
              ...
              onChange={(e) => {
                ...
                setNewUser({ ...newUser, role, permissions: newUser.permissions.filter((m) => allowedIds.includes(m)) });
              }}
              required
            >
          ...
          <label className="flex items-start gap-2.5 bg-teal-50 border border-teal-200 rounded-lg p-3 cursor-pointer">
            <input
              ...
              onChange={(e) => setNewUser({ ...newUser, restrictAccess: e.target.checked, permissions: e.target.checked ? newUser.permissions : [] })}
              className="mt-0.5"
            />
            <span className="text-xs text-teal-700">
              Restrict this user to specific modules only (e.g. tick just "IP Management" so that's all they can see).
            </span>
          </label>
          {newUser.restrictAccess && (
            <ModuleAccessChecklist role={newUser.role} selected={newUser.permissions} onToggle={toggleNewUserModule} />
          )}
          ...
          <label className="flex items-start gap-2.5 bg-brand-50 dark:bg-brand-900/40 border border-brand-200 rounded-lg p-3 cursor-pointer">
            ...
            <span className="text-xs text-brand-700 dark:text-brand-300">
              Force this user to change their password the first time they log in (recommended for new accounts).
            </span>
          </label>
      ...
      <Modal open={editModalOpen} onClose={() => setEditModalOpen(false)} title="Edit User" size="md">
        <form noValidate onSubmit={handleEditUser} className="space-y-4">
          ...
          <Field label="Email Address" required hint="Changing this updates the address the user logs in with.">
            <TextInput
              type="email"
          ...
          <Field label="Role" required>
            <SelectInput
              ...
              onChange={(e) => {
                const role = e.target.value as UserRole;
                const allowedIds = modulesForRole(role).map((m) => m.id);
                setEditForm({ ...editForm, role, permissions: editForm.permissions.filter((m) => allowedIds.includes(m)) });
              }}
              required
              disabled={editingUser?.id === profile?.id || editingUser?.is_owner}
            >
              {roleOptions.map((r) => <option key={r.value} value={r.value}>{r.label} — {r.hint}</option>)}
            </SelectInput>
            {editingUser?.id === profile?.id && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">You cannot change your own role.</p>
            )}
            {editingUser?.is_owner && editingUser?.id !== profile?.id && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">The owner's role cannot be changed.</p>
            )}
          </Field>
            ...
            <>
              <label className="flex items-start gap-2.5 bg-teal-50 border border-teal-200 rounded-lg p-3 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setEditForm({ ...editForm, restrictAccess: e.target.checked, permissions: e.target.checked ? editForm.permissions : [] })}
                  className="mt-0.5"
                />
                <span className="text-xs text-teal-700">
                  Restrict this user to specific modules only (e.g. tick just "IP Management" so that's all they can see).
              ...
              {editForm.restrictAccess && (
                <ModuleAccessChecklist role={editForm.role} selected={editForm.permissions} onToggle={toggleEditModule} />
              )}
            </>
          )}
          <Field label="Phone">
      ...
      <Modal open={resetModalOpen} onClose={() => setResetModalOpen(false)} title={`Reset Password${resettingUser ? ` — ${resettingUser.full_name}` : ''}`} size="sm">
        <form noValidate onSubmit={handleResetPassword} className="space-y-4">
          <Field label="New Temporary Password" required hint="At least 8 characters, with upper/lowercase, a number, and a symbol. Can't be the same as their current password. Share this with the user securely.">
            <TextInput
              type="password"
          ...
          <label className="flex items-start gap-2.5 bg-brand-50 dark:bg-brand-900/40 border border-brand-200 rounded-lg p-3 cursor-pointer">
            ...
            />
            <span className="text-xs text-brand-700 dark:text-brand-300">
              Force the user to set their own new password the next time they log in (recommended).
            </span>
          </label>
          <div className="flex justify-end gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
            <Button type="button" variant="secondary" onClick={() => setResetModalOpen(false)}>Cancel</Button>
            <Button type="submit" variant="primary" disabled={saving}>{saving ? 'Resetting...' : 'Reset Password'}</Button>
          </div>
        </form>
      </Modal>

      <Modal open={transferModalOpen} onClose={() => setTransferModalOpen(false)} title={`Transfer Ownership${transferTarget ? ` — ${transferTarget.full_name}` : ''}`} size="sm">
        <form noValidate onSubmit={handleTransferOwnership} className="space-y-4">
          <div className="flex items-start gap-2.5 bg-red-50 border border-red-200 rounded-lg p-3 text-xs text-red-700">
            ...
            <span>
              {transferTarget?.full_name} will become the sole owner — protected from deletion, demotion,
              and being disabled by any other admin. You will lose that protection and become a regular admin.
              This cannot be undone by yourself; only the new owner could transfer it back.
            </span>
          </div>
          <Field label="Confirm Your Password" required hint="Re-enter your password to confirm this change.">
            <TextInput
              type="password"
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
            <Button type="button" variant="secondary" onClick={() => setTransferModalOpen(false)}>Cancel</Button>
            <Button type="submit" variant="primary" disabled={transferring || !transferPassword}>

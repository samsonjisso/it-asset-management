...
export async function listNotifications(auth: AuthContext, searchParams: URLSearchParams) {
  ...
  params.push(limit);
  const [rows] = await pool.query<any[]>(`SELECT * FROM notifications ${where} ORDER BY created_at DESC LIMIT ?`, params);
  return rows.map(toJson);
}

export async function unreadCount(auth: AuthContext) {
  assertViewAccess(auth);
  const [rows] = await pool.query<any[]>('SELECT COUNT(*) as c FROM notifications WHERE is_read = 0');
  return { count: rows[0].c };
}

export async function markRead(auth: AuthContext, id: string, isRead: boolean) {
  assertViewAccess(auth);
  const [result]: any = await pool.query('UPDATE notifications SET is_read = ? WHERE id = ?', [isRead ? 1 : 0, id]);
...
export async function deleteNotification(auth: AuthContext, id: string) {
  ...
  const [result]: any = await pool.query('DELETE FROM notifications WHERE id = ?', [id]);
  if (result.affectedRows === 0) throw new ApiError(404, 'Not found');
  return { ok: true };
}
